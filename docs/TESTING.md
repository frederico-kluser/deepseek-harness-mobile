...
>
> **As suítes novas entram no gate sem comando extra** — os testes do registry
> de agentes vivem em `test/unit/**` e correm no `pnpm
> test` normal. Roteiro rápido da
> onda: `pnpm test -- --test-name-pattern "agents"`.
...
| E2E offline | `pnpm test:e2e` | `test/e2e/**`, processos/sockets reais | ciclo de vida, túnel com fake-cloudflared e o worker telegram (`test/e2e/telegram-*.test.ts` — processo real contra o duble da Bot API) |
...
- `test/unit/worker/providers/**` — os **adaptadores** e o registry: `registry.test.ts` (a
  tabela fechada e o `ProvedorDesconhecidoError`) e `providers/telegram/**` (parse, teclado,
  token, transporte, cliente, polling, adapter) contra o **duble local**
  (`test/support/telegram-server.mjs`);
