 ...
 */
export function assistenteDeEvento(evento: HarnessSessionEvent): DadosDoAssistant | undefined {
  if (evento.type !== 'assistant/message') return undefined
  const data: unknown = evento.data
  if (typeof data !== 'object' || data === null) return undefined
  return data as DadosDoAssistant
}
...
type CampoDeUso = keyof UsoDobrado

const CAMPOS_DE_USO: readonly CampoDeUso[] = [
  'inputTokens',
  'outputTokens',
  'cacheReadTokens',
  'cacheWriteTokens',
 ...
 */
export function dobrarUsageDosEventos(eventos: readonly HarnessSessionEvent[]): UsoDobrado {
  const somas: Record<CampoDeUso, number> = {
    inputTokens: 0,
    outputTokens: 0,
    cacheReadTokens: 0,
    cacheWriteTokens: 0,
  }
  const visto: Record<CampoDeUso, boolean> = {
    inputTokens: false,
    outputTokens: false,
  ...
  const saida: UsoDobrado = {}
  for (const campo of CAMPOS_DE_USO) {
    if (visto[campo]) saida[campo] = somas[campo]
  }
  return saida
}

const CAMPOS_DE_ESTATISTICAS: readonly (keyof HarnessSessionStatsProjection)[] = [
  'turns',
  'steps',
  'llmMs',
  'toolMs',
 ...
 */
export function metricasDasEstatisticas(
  stats: HarnessSessionStatsProjection | undefined,
): Partial<AgentRunMetrics> {
  const saida: { -readonly [K in keyof HarnessSessionStatsProjection]?: number } = {}
  if (stats === undefined) return saida
...
export function criarColetorDeMetricas(deps: MetricasDeps): ColetorDeMetricas {
  return {
    daSessao(sessionId) {
      ...
      try {
        ...
        const sessao: HarnessSession | undefined = sessoes.get(sessionId)
        ...
        const metricas: AgentRunMetrics = { ...uso, ...estatisticas }
        ...
        deps.log.warn(
          `metricas da sessao ${sessionId} ilegiveis: ${error instanceof Error ? error.message : String(error)}`,
        )
        return undefined
      }
    },
