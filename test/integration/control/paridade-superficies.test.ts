...
describe('CTL-040 (S4): paridade das tres superficies', () => {
  it('a lista canonica (D5) tem par de controlo nas tres superficies', () => {
    ...
    // ONDA-1-NOME-E-BOTOES (Tarefa 3): `status`/`emergencia` saem do menu
    // publicado — ficam SO como botoes do cartao; os comandos digitados seguem
    // validos (routing intacto). As tres superficies seguem a mesma lista.
    assert.deepEqual(comandos, ['menu', 'parear', 'ajuda'])
