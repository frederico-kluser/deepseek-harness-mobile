 ...
 */
export function createUlidFactory(now, random = () => randomBytes(RANDOM_BYTES)) {
    let lastMs = -1;
    let lastRandom = 0n;
    return function ulid() {
        ...
        if (bytes.length < RANDOM_BYTES) {
            throw new Error(`aleatoriedade do ULID curta demais: ${String(bytes.length)} bytes (minimo ${String(RANDOM_BYTES)})`);
        }
        let aleatorio = 0n;
        ...
        if (ms === lastMs) {
            ...
            if (aleatorio === 0n)
                ms += 1; // a parte aleatoria enrolou: sobe o ms
        }
        lastMs = ms;
