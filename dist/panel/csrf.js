...
export function createCsrfGuard(deps) {
    ...
    if (key.length < 16) {
        throw new Error(`[${PLUGIN_NAME}] chave de CSRF curta demais: ${key.length} bytes (minimo 16)`);
    }
    ...
    return {
        ...
        verify(token, binding) {
            ...
            if (!EXPIRY_SHAPE.test(rawExpiry))
                return false;
