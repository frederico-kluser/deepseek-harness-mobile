 ...
 */
export const TUNNEL_STATE_LABEL = Object.freeze({
    STOPPED: 'desligado',
    STARTING: 'ligando',
    READY: 'online',
 ...
 */
export const CONTROL_RECUSA_LABEL = Object.freeze({
    SHUTDOWN_IN_PROGRESS: 'O túnel está a desligar. Espere terminar e tente outra vez.',
    MODO_RESTRITO: 'Modo restrito ativo: o DSH recusa ligar o túnel até ser desbloqueado na máquina.',
    SEM_SEGREDO_FORTE: 'Não há segredo forte configurado — o túnel não pode ligar.',
    TERMINAL_SEM_RESET: 'O túnel está num estado terminal (falhou) — reinicie o DSH para recuperar.',
    NONCE_AUSENTE: 'Confirmação ausente — use o botão de duas etapas.',
    NONCE_INVALIDO: 'Confirmação inválida ou já usada — toque em Ligar outra vez para obter uma nova.',
    NONCE_EXPIRADO: 'Confirmação expirada (60 s) — toque em Ligar outra vez para obter uma nova.',
});
 ...
 */
export function panelHtmlHeaders(nonce) {
    return {
...
function renderDocument(input) {
    const scriptBlock = input.script.length === 0
        ? ''
 ...
 */
export function renderPanelPage(input) {
