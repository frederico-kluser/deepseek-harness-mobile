...
import { CREDENTIAL_DENIED_RESPONSE, emitSession, INTERNAL_ERROR_RESPONSE, OK_JSON_RESPONSE, } from "./api.js";
import { newNonce, panelHtmlHeaders, renderMagicPage } from "./html.js";
 ...
 * cookie duplicado.
 */
export function classifyClickSignal(req) {
    const bruto = req.headers[CLICK_SIGNAL_HEADER];
    if (bruto === undefined)
        return 'ausente';
    if (typeof bruto !== 'string')
        return 'de-outro-sitio';
 ...
 */
export function createMagicPageHandler(bindings) {
 ...
 */
export function createMagicConsumeHandler(deps) {
    return async (exchange) => {
        ...
        const outcome = await runThrottledAttempt(deps.limiter, exchange.identity, () => deps.magic.consume(mk), deps.wait);
        if (!outcome.granted) {
            deps.audit.append({
                evento: 'painel_magic',
                resultado: 'negado',
