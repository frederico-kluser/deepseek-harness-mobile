 ...
 */
import { canonicalRequestPath, isGuardedPath } from "../http/path.js";
import { denyUnauthorized } from "../http/responses.js";
import { maskAuditText } from "../audit/format.js";
import { readSessionCookie } from "../session/cookie.js";
import { createAuditGate, createLoginHandler, createPanelPageHandler, createStateHandler, createTunnelNonceHandler, createTunnelStartHandler, createTunnelStopHandler, emptyFields, FORBIDDEN_RESPONSE, INTERNAL_ERROR_RESPONSE, MAX_BODY_BYTES, NOT_FOUND_RESPONSE, PAYLOAD_TOO_LARGE_RESPONSE, readRequestBody, } from "./api.js";
import { CSRF_FIELD_NAME, CSRF_HEADER_NAME } from "./csrf.js";
import { createMagicConsumeHandler, createMagicPageHandler } from "./magic.js";
import { createSecretHandler } from "./secret.js";
/* ========================================================================== */
 ...
 * declara.
 */
export function routeKeyOf(method, path) {
...
export const PANEL_ROUTE_POLICY = new Map(TABELA_DE_POLITICA);
/**
 ...
 * acusaria.
 */
export function policyForRouteKey(key) {
...
export function panelPublicRouteKeys() {
    const keys = [];
 ...
 */
export function panelRoutes(deps, audit) {
...
export function writePanelResponse(res, response) {
...
function presentedCsrfToken(req, fields) {
 ...
 */
export function createPanelRouter(deps, routes = panelRoutes) {
    ...
    const tabelaDeRotas = routes(deps, audit).map((rota) => ({
        ...rota,
        path: canonicalRequestPath(rota.path),
    }));
    const dispatch = async (req, res, path) => {
        ...
        const route = tabelaDeRotas.find((candidate) => candidate.method === method && candidate.path === path);
        if (route === undefined) {
            writePanelResponse(res, NOT_FOUND_RESPONSE);
            return;
        }
    ...
    return async (req, res) => {
        ...
        try {
            ...
            deps.log.error(maskAuditText(`[painel] excecao ao servir ${String(req.method)} ${path}: ` +
