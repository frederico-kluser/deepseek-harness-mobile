...
import { MAX_RUNS_HISTORY } from "../agents/registry.js";
import { PLUGIN_NAME } from "../errors.js";
import { MAX_RUNS_PER_REPORT } from "../ipc/channel.js";
import { PROVIDER_ENV } from "../proc/env.js";
import { resolveWorkerCwd } from "./schema.js";
/**
...
const TUNNEL_MODES = ['quick', 'named'];
/** Os dois unicos modos de exposicao. */
const EXPOSURE_MODES = ['loopback', 'tunnel'];
/**
 ...
 * para os dois lados.
 */
function assertUsableCredential(encodedAuthString, path) {
    const decoded = Buffer.from(encodedAuthString, 'base64').toString('utf8');
    const separator = decoded.indexOf(':');
    if (separator === -1) {
        throw new Error(`[${PLUGIN_NAME}] config.${path} tem de ser base64 de 'utilizador:senha' (RFC 7617). ` +
    ...
    if (user.length === 0 || pass.length === 0) {
        throw new Error(`[${PLUGIN_NAME}] config.${path}: o par descodificado tem utilizador ou senha vazios. ` +
    ...
    for (const [label, part] of [
        ...
        if (PLACEHOLDER_CREDENTIAL_PARTS.has(part.toLowerCase())) {
            throw new Error(`[${PLUGIN_NAME}] config.${path}: o ${label} descodificado e o literal '${part}'. ` +
...
function assertBoolean(value, path) {
    if (typeof value !== 'boolean') {
        throw new Error(`[${PLUGIN_NAME}] config.${path} tem de ser um booleano (recebido: ${typeof value}). ` +
...
function assertPositiveInteger(value, path) {
    if (typeof value !== 'number' || !Number.isInteger(value) || value <= 0) {
        throw new Error(`[${PLUGIN_NAME}] config.${path} tem de ser um inteiro positivo (recebido: ${String(value)}).`);
    }
}
/**
 ...
 */
export function assertExposureConfig(value, path) {
    if (typeof value !== 'object' || value === null) {
        throw new Error(`[${PLUGIN_NAME}] config.${path} tem de ser um objeto.`);
    }
    const exposure = value;
    if (typeof exposure['mode'] !== 'string' || !EXPOSURE_MODES.includes(exposure['mode'])) {
        throw new Error(`[${PLUGIN_NAME}] config.${path}.mode tem de ser 'loopback' ou 'tunnel' ` +
    ...
    if (exposure['trustEdgeHeaders'] === true && exposure['mode'] !== 'tunnel') {
        throw new Error(`[${PLUGIN_NAME}] config.${path}.trustEdgeHeaders so pode ser true com ` +
 ...
 */
export function assertTunnelConfig(value, path) {
    if (typeof value !== 'object' || value === null) {
        throw new Error(`[${PLUGIN_NAME}] config.${path} tem de ser um objeto.`);
    }
    const tunnel = value;
    if (typeof tunnel['mode'] !== 'string' || !TUNNEL_MODES.includes(tunnel['mode'])) {
        throw new Error(`[${PLUGIN_NAME}] config.${path}.mode tem de ser 'quick' ou 'named' ` +
    ...
    if (typeof ttl !== 'number' || !Number.isInteger(ttl) || ttl <= 0 || ttl > TUNNEL_TTL_MAX_MINUTES) {
        throw new Error(`[${PLUGIN_NAME}] config.${path}.ttlMinutes tem de ser um inteiro entre 1 e ` +
    ...
    if (tunnel['binaryPath'] !== undefined)
        assertNonEmptyString(tunnel['binaryPath'], `${path}.binaryPath`);
    if (tunnel['tokenFile'] !== undefined)
        assertNonEmptyString(tunnel['tokenFile'], `${path}.tokenFile`);
    ...
    if (tunnel['mode'] === 'named' && tunnel['tokenFile'] === undefined) {
        throw new Error(`[${PLUGIN_NAME}] config.${path}.mode = 'named' exige config.${path}.tokenFile ` +
    ...
    if (tunnel['metricsPort'] !== undefined) {
        ...
        if (tunnel['metricsPort'] > 65535) {
            throw new Error(`[${PLUGIN_NAME}] config.${path}.metricsPort tem de ser <= 65535.`);
        }
    }
    if (tunnel['graceMs'] !== undefined)
        assertPositiveNumber(tunnel['graceMs'], `${path}.graceMs`);
    if (tunnel['backoff'] !== undefined) {
        ...
        if (b['maxDelayMs'] < b['initialDelayMs']) {
            throw new Error(`[${PLUGIN_NAME}] config.${path}.backoff.maxDelayMs nao pode ser inferior a initialDelayMs.`);
        }
    }
}
/**
...
export function assertControlConfig(value, path) {
    if (typeof value !== 'object' || value === null) {
        throw new Error(`[${PLUGIN_NAME}] config.${path} tem de ser um objeto.`);
    }
    assertBoolean(value['requireConfirmation'], `${path}.requireConfirmation`);
}
/**
 ...
 */
export function assertAgentsConfig(value, path) {
    if (typeof value !== 'object' || value === null) {
        throw new Error(`[${PLUGIN_NAME}] config.${path} tem de ser um objeto.`);
    }
    const agents = value;
    assertStringArray(agents['skills'], `${path}.skills`);
    for (const [indice, skill] of agents['skills'].entries()) {
        if (!SKILL_NAME.test(skill)) {
            throw new Error(`[${PLUGIN_NAME}] config.${path}.skills[${String(indice)}] = '${skill}' nao e um nome ` +
    ...
    if (typeof maxRuns !== 'number' || !Number.isInteger(maxRuns) || maxRuns < 1) {
        throw new Error(`[${PLUGIN_NAME}] config.${path}.maxRuns tem de ser um inteiro >= 1 ` +
    ...
    if (maxRuns > AGENTS_MAX_RUNS_CEILING) {
        throw new Error(`[${PLUGIN_NAME}] config.${path}.maxRuns tem de ser um inteiro entre 1 e ` +
 ...
 */
function assertExistingDirectory(value, path) {
    let isDirectory;
    try {
        ...
        throw new Error(`[${PLUGIN_NAME}] config.${path} ('${value}') nao existe ou nao e acessivel: ${reason}. ` +
            'O spawn do worker falharia com ENOENT a cada tentativa.', { cause: error });
    }
    if (!isDirectory) {
...
export function assertValidConfig(config) {
    ...
    if (config.allowedHosts.length === 0) {
        throw new Error(`[${PLUGIN_NAME}] config.allowedHosts nao pode estar vazio (nenhum bind seria valido).`);
    }
    ...
    for (const [index, prefix] of config.guardedPrefixes.entries()) {
        if (!prefix.startsWith('/')) {
            throw new Error(`[${PLUGIN_NAME}] config.guardedPrefixes[${index}] = '${prefix}' tem de comecar por '/'. ` +
    ...
    const token = config.worker.token;
    if (token !== undefined && typeof token !== 'string') {
        throw new Error(`[${PLUGIN_NAME}] config.worker.token tem de ser uma string (recebido: ${typeof token}). ` +
    ...
    if (config.worker.provider !== undefined) {
        if (!(config.worker.provider in PROVIDER_ENV)) {
            throw new Error(`[${PLUGIN_NAME}] config.worker.provider = '${String(config.worker.provider)}' nao e um ` +
    ...
    if (backoff.maxDelayMs < backoff.initialDelayMs) {
        throw new Error(`[${PLUGIN_NAME}] config.worker.backoff.maxDelayMs (${backoff.maxDelayMs}) ` +
    ...
    if (config.exposure !== undefined)
        assertExposureConfig(config.exposure, 'exposure');
    if (config.tunnel !== undefined)
        assertTunnelConfig(config.tunnel, 'tunnel');
    if (config.control !== undefined)
        assertControlConfig(config.control, 'control');
    if (config.agents !== undefined)
        assertAgentsConfig(config.agents, 'agents');
    ...
    if (config.exposure?.mode === 'tunnel' && config.tunnel === undefined) {
        throw new Error(`[${PLUGIN_NAME}] config.exposure.mode = 'tunnel' exige o objeto config.tunnel ` +
