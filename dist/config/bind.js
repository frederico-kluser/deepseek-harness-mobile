 ...
 * politica.
 */
export const KNOWN_BIND_HOSTS = ['127.0.0.1', '0.0.0.0'];
const everyBindHostIsCovered = true;
...
export function isWildcardBindHost(host) {
    ...
    if (WILDCARD_BIND_HOSTS.has(value))
        return true;
 ...
 */
export function assertSecureBind(host, allowedHosts) {
    if (isWildcardBindHost(host)) {
        throw new Error(`[${PLUGIN_NAME}] Bind inseguro: ctx.webServer.host = '${host}'. ` +
    ...
    if (!allowedHosts.includes(host)) {
        throw new Error(`[${PLUGIN_NAME}] Bind nao autorizado: ctx.webServer.host = '${host}' ` +
