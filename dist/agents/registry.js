 ...
 * =============================================================================
 */
import { comporEventoAgenteCancelar, comporEventoAgenteDespacho, comporEventoAgenteFim, } from "../audit/events.js";
import { MAX_RUNS_PER_REPORT } from "../ipc/channel.js";
import { createUlidFactory } from "../ulid.js";
import { ChatError, promptDeChatValido } from "./chats.js";
import { WorktreeError, nomeDeWorktreeValido, } from "./worktrees.js";
import { renderHarnessSkill } from "./harness.js";
...
export function createAgentRegistry(deps) {
    ...
    const runs = [];
    const permitida = (skill) => deps.skillsPermitidas.includes(skill);
    const contando = (runsAtivos) => runsAtivos >= deps.maxRuns;
    /**
     ...
     */
    const relatorio = () => runs.slice(-MAX_RUNS_PER_REPORT).map((run) => {
    ...
    const difundir = () => {
        try {
            ...
            log.error(`difusao de agent.report falhou: ${error instanceof Error ? error.message : String(error)}`);
        }
    };
    /**
     ...
     */
    const encerrar = (run, status, summary) => {
        run.status = status;
     ...
     */
    const executar = async (run, skill, prompt) => {
        try {
            ...
            try {
                ...
                encerrar(run, 'failed', `falha de infraestrutura do harness: ${error instanceof Error ? error.message : String(error)}`);
                return;
            }
    ...
    const podarHistorico = () => {
        while (runs.length > MAX_RUNS_HISTORY) {
            ...
            if (maisAntigo === undefined || maisAntigo.status === 'running')
                break;
            runs.shift();
        }
    };
     ...
     */
    const executarChat = async (run, pedido) => {
        ...
        try {
            ...
            encerrar(run, terminal, terminal === 'done'
                ? resposta
            ...
            log.error(`tarefa de chat ${run.id} falhou: ${error instanceof Error ? error.message : String(error)}`);
     ...
     */
    const executarWorktree = async (run, pedido) => {
        ...
        try {
            const criada = await worktrees.criar({ nome: pedido.nome, ...(pedido.base === undefined ? {} : { base: pedido.base }) }, { signal: run.abortar.signal });
            encerrar(run, 'done', `worktree "${criada.nome}" criada (branch ${criada.branch}, a partir de ${criada.base}).`);
        }
        catch (error) {
            if (run.status !== 'running')
                return;
            ...
            log.error(`tarefa de worktree ${run.id} falhou: ${error instanceof Error ? error.message : String(error)}`);
     ...
     */
    const registarTarefa = (skill, kind, origem, worktree, executar) => {
        const run = {
            id: ulid().slice(-8),
            skill,
            origem,
    ...
    return {
        despachar(pedido) {
            ...
            if (!permitida(pedido.skill)) {
                ...
                log.warn(`despacho de agente RECUSADO: a skill '${pedido.skill}' nao esta na allowlist ` +
            ...
            if (contando(ativos)) {
                ...
                log.warn(`despacho de agente RECUSADO: ${String(ativos)} runs ativos atingem o teto ` +
            ...
            if (deps.subagents() === undefined || deps.agentesDoHarness() === undefined || deps.skillsDoHarness() === undefined) {
                log.warn('despacho de agente RECUSADO: os servicos do harness (subagents/agents/skills) ' +
        ...
        novoChat(pedido) {
            ...
            if (contando(ativos)) {
                ...
                log.warn(`tarefa de chat RECUSADA: ${String(ativos)} runs ativos atingem o teto ` +
                    `config agents.maxRuns (${String(deps.maxRuns)}).`);
                return { ok: false, motivo: 'teto-atingido' };
            }
            ...
            registarTarefa(ROTULO_TAREFA_CHAT, 'chat', pedido.origem, pedido.worktree, (run) => executarChat(run, pedido));
            return { ok: true };
        },
        novoWorktree(pedido) {
            ...
            try {
                ...
                log.warn(`tarefa de worktree RECUSADA: nao foi possivel inspecionar o caminho: ` +
            ...
            if (contando(ativos)) {
                ...
                log.warn(`tarefa de worktree RECUSADA: ${String(ativos)} runs ativos atingem o teto ` +
                    `config agents.maxRuns (${String(deps.maxRuns)}).`);
                return { ok: false, motivo: 'teto-atingido' };
            }
            // 5. ACEITE: run `kind: 'worktree'` com o NOME no campo `worktree`.
            registarTarefa(ROTULO_TAREFA_WORKTREE, 'worktree', pedido.origem, pedido.nome, (run) => executarWorktree(run, pedido));
            return { ok: true };
        },
        estado() {
            return relatorio();
        ...
        cancelar(agentId, origem) {
            ...
            if (alvo.sessao !== undefined)
                deps.chats?.()?.cancelar(alvo.sessao);
        ...
        dispose() {
            ...
            for (const run of runs.toReversed()) {
                ...
                if (run.sessao !== undefined)
                    deps.chats?.()?.cancelar(run.sessao);
