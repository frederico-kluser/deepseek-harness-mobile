...
import { classifyNonRetryable } from "./failure.js";
import { createRestartBudget } from "./retry.js";
import { defaultClockDeps } from "./scheduler.js";
import { attachStreamLogging } from "./stream-log.js";
import { treeKill } from "./tree-kill.js";
export { createWorkerSupervisor, } from "./worker.js";
/** Dependencias reais (processo Node corrente). */
export const defaultSupervisorDeps = {
    ...defaultClockDeps,
...
export function createProcessSupervisor(ctx, target, deps = defaultSupervisorDeps) {
    const { name } = target;
    ...
    const isCancelled = () => disposed || abortController.signal.aborted;
    /**
    ...
    function spawnOnce() {
        if (isCancelled() || budget.exhausted)
            return;
         ...
         */
        const handleTermination = (description, cause) => {
            if (settled)
                return;
            settled = true;
            detachStreamListeners?.();
        ...
        void spawned.done.then((outcome) => {
            handleTermination(`${name} encerrado (code=${String(outcome.exitCode)} signal=${String(outcome.signal)}).`);
        }, (error) => {
    ...
    const restart = (reason) => {
        if (isCancelled() || budget.exhausted || !started)
            return;
 ...
 */
function coerceSpecError(error) {
    if (classifyNonRetryable(error) !== undefined)
        return error;
    const detail = error instanceof Error ? error.message : String(error);
