...
import { assertRateLimitPolicy, banAppliesToScope, computeAuthDelayMs, hasReachedBruteForceCeiling, isBanTriggeredBy, resolveIdentityBucket, } from "./policy.js";
...
export { GLOBAL_BUCKET_KEY, resolveIdentityBucket, } from "./policy.js";
export const nodeIntervalScheduler = {
...
export function createFailureTracker(deps) {
    assertRateLimitPolicy(deps.policy);
    if (!Number.isInteger(deps.maxTrackedIdentities) || deps.maxTrackedIdentities <= 0) {
        throw new Error(`[${PLUGIN_NAME}] RATE_LIMIT_TRACKER_INVALID: maxTrackedIdentities tem de ser inteiro positivo`);
    }
    ...
    const assertLive = () => {
        if (disposed) {
            throw new Error(`[${PLUGIN_NAME}] RATE_LIMIT_TRACKER_DISPOSED: o limitador ja foi disposto; ` +
                'usar um limitador morto e aceitar tudo em silencio');
        }
    };
    /** Um balde so pode ser esquecido quando nao ha ban vivo NEM janela viva. */
    const isForgettable = (counter, at) => at >= counter.bannedUntil && at - counter.lastFailureAt > deps.policy.observationWindowMs;
    const prune = (at) => {
        for (const [key, counter] of counters) {
            if (isForgettable(counter, at))
                counters.delete(key);
        }
    };
    /**
     ...
     */
    const evictIfNeeded = (at) => {
        if (counters.size < deps.maxTrackedIdentities)
            return;
        let scanned = 0;
        let oldest;
        for (const [key, counter] of counters) {
    ...
    const effectiveFailures = (counter, at) => isForgettable(counter, at) ? 0 : counter.failures;
    ...
    const recordFailure = (identity) => {
        ...
        if (banAppliesToScope(bucket.scope) &&
 ...
 */
export async function runThrottledAttempt(tracker, identity, verify, wait) {
    const decision = tracker.check(identity);
    await wait(decision.retryAfterMs);
    const verified = verify();
    if (verified) {
        if (decision.allowed)
            tracker.recordSuccess(identity);
        else
            tracker.recordVerifiedButDenied(identity);
