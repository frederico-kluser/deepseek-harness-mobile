...
import { AUDIT_FILE_NAME, openAuditLog } from "../src/audit/log.js";
import { redact } from "../src/logging/redact.js";
import { AVISOS_ANTES_DO_TUNEL, COMANDO_CLI, LIMITE_DE_UPDATES, OnboardingError, analisarArgumentos, caminhoApresentavel, caminhoDoSecretsEnv, criarSondaHttp, gravarSecretsEnv, proximoPasso, resolverToken, validarFormatoDoToken, } from "../src/telegram/onboarding.js";
import { PROVIDER_ENV, resolverProvedorDoAmbiente } from "../src/proc/env.js";
import { PairingError, TTL_DO_CODIGO_MS, criarSessaoDePareamento, } from "../src/telegram/pairing.js";
import { createSecretStore } from "../src/secret/store.js";
import { ensureStateDir, resolveStatePaths } from "../src/state/paths.js";
...
escreverErro = (texto) => void process.stderr.write(texto)) {
...
export { gravarSecretsEnv } from "../src/telegram/onboarding.js";
/**
 ...
 */
async function recolherRetrato(ctx, ambiente) {
    const dono = ctx.store.read().pairing;
    // A CHAVE e a do provedor ativo (`TELEGRAM_BOT_TOKEN`) — as duas fontes
    // (secrets.env e ambiente) usam o MESMO nome.
    ...
    const configurado = { origem: encontrado.origem, formato };
    if (!formato.valido) {
        return { retrato: { token: configurado, getMe: undefined, dono }, token: encontrado.token };
    }
    const getMe = await getMeDoProvedor(ctx, encontrado.token);
 ...
 * byte a byte. Um provedor novo acrescenta aqui o seu ramo e o mapeamento
 * para `RespostaGetMe` (o retrato so consome `ok`/`falha.causa`/`httpStatus`
 * para ESCOLHER TEXTO).
 */
async function getMeDoProvedor(ctx, token) {
 ...
 */
async function lerLinha(rotulo, ecoar) {
    ...
    const rl = createInterface(ecoar
...
function mostrarPasso(retrato, ctx, codigo) {
 ...
 * ser enviada.
 */
async function comandoPedirToken(ctx, ambiente) {
    const origemDaChave = 'que o @BotFather lhe deu';
    ...
    if (!formato.valido) {
        ...
        return mostrarPasso({ token: { origem: 'secrets.env', formato }, getMe: undefined, dono: ctx.store.read().pairing }, ctx);
    }
    // A CHAVE gravada e a do provedor ativo (`ctx.chave`): o `secrets.env`
    // partilhado guarda a linha de cada provedor, e o worker de cada provedor
    // le a sua.
 ...
 */
async function comandoParear(ctx, token, retrato, confirmado) {
    ...
    for (;;) {
        ...
        ctx.escrever('Enquanto isto espera, o harness não deve estar a correr com este mesmo bot:\n' +
        ...
        ctx.escrever(resultado === 'esgotado'
 ...
 */
async function esperarPareamento(ctx, sonda, token, sessao, vistos) {
    /*
    ...
    if (sessao.estado() === 'consumido')
        return 'pareado';
    return sessao.estado() === 'esgotado' ? 'esgotado' : 'expirado';
}
function identificadorDoUpdate(update) {
 ...
 */
async function comandoResetPairing(ctx, token) {
    const dono = ctx.store.read().pairing;
    if (dono === undefined) {
 ...
 * `sessions` abaixo.
 */
function mostrarSegredoSeFaltar(ctx) {
    const segredos = createSecretStore({
        ...
        sessions: {
             ...
             */
            revokeAll: () => {
                throw new Error('a rotação da senha tem de ser feita pelo harness a correr, porque só ele ' +
 ...
 */
function registarAuditoria(ctx, token, evento, resultado) {
...
/* Ajuda                                                                      */
/* ========================================================================== */
function mostrarAjuda(escrever, _provedor) {
...
export async function principal(argv, deps = {}) {
    const escrever = deps.escrever ?? ((texto) => void process.stdout.write(`${texto}\n`));
    const avisar = deps.avisar ?? ((texto) => void process.stderr.write(`${texto}\n`));
    ...
    // provedor tem a sua linha (`TELEGRAM_BOT_TOKEN`).
 ...
 */
const somosOPrograma = process.argv[1] !== undefined && import.meta.url === `file://${process.argv[1]}`;
if (somosOPrograma) {
    try {
...
//# sourceMappingURL=dsh-guard-setup.js.map