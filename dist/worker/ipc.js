...
import { IPC_PROTOCOL_VERSION, } from "../src/contracts/ipc.js";
/* ========================================================================== */
...
/** Ids de identidade (`from`/`chat`) — STRING em V2 (EMENDA ONDA-1-IPC-ENVELOPE-STRING). A politica minima espelha o `normalizeKey` do worker (`./surface/ids.ts`): trim + nao vazio. NAO se valida FORMATO de provedor nenhum (um id nao-numerico nao e `[0-9]+` curto, um id de Matrix e uma url) — so se exige uma string utilizavel. A guarda de controlo impede ruido de terminal; o teto impede uma linha gigante. */
function isId(value) {
    return (typeof value === 'string' &&
        value.trim() !== '' &&
 ...
 */
export function validateWorkerIpcMessage(value, direction) {
...
export function parseWorkerIpcLine(line, direction) {
    let parsed;
    try {
 ...
 */
export function serializeWorkerIpcMessage(message, direction) {
 ...
 */
export function createWorkerIpc(options) {
    ...
    if (options.env !== undefined && options.env[WORKER_IPC_ENV_VAR] !== '1') {
        log(`${WORKER_IPC_ENV_VAR} ausente: este processo nao foi arrancado pelo plugin. ` +
            'O canal IPC continua armado e o EOF do stdin continua a termina-lo.');
    }
    return {
        log,
        send: (message) => {
            if (disposed || disconnected || !output.writable)
                return false;
            let line;
            try {
            ...
            if (output.writableLength > maxPendingBytes) {
                ...
                if (!saturado) {
                    saturado = true;
                    log(`o host parou de ler (${String(output.writableLength)} bytes por escrever): ` +
 ...
 * matar o corredor de testes.
 */
export function bindWorkerIpcToProcess(proc, options) {
    let terminado = false;
    /**
