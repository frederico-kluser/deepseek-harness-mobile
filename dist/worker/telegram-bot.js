...
import { systemTime } from "./lib/clock.js";
import { createWorkerLogger } from "./lib/log.js";
import { bindWorkerIpcToProcess } from "./ipc.js";
import { ProvedorDesconhecidoError, criarPonteDeNonce, criarSurfaceIpcBridge, resolverProvedor, } from "./providers/registry.js";
import { isWorkerExitCode, WORKER_EXIT } from "./lib/errors.js";
import { describeForLog } from "./lib/redact.js";
import { criarAuthDeSuperficie, criarDesafioDePareamento } from "./surface/auth.js";
import { criarComandosDeSuperficie, COMANDOS_PUBLICADOS } from "./surface/commands.js";
import { criarNucleo } from "./surface/core.js";
/**
...
function montarDoAdaptador(adapter) {
 ...
 */
export async function runTelegramWorker(runtime = {}) {
    ...
    let token;
    const segredosDe = () => (token === undefined ? [] : [token]);
    ...
    const despachar = (msg) => {
        ...
        switch (msg.type) {
            ...
            case 'nonce.issued':
                ...
                return;
            case 'agent.report':
                // EMENDA ONDA-5-AGENTS-SUPERFICIE: a lista de runs chega (resposta a
                // /agentes e difusao proativa quando um run termina). O NUCLEO decide:
                // com `agent.status` pendente renderiza a lista como resposta; sem
                // pendente, notifica o dono com os runs que terminaram (S4: nunca
                // derruba o canal).
                n.onAgentReport(msg);
                return;
        }
 ...
 */
export const WORKER_LOG_LEVEL = 'debug';
/**
