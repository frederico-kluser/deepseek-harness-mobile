...
export const ROTULOS_DE_ESTADO = Object.freeze({
    STOPPED: 'desligado',
    STARTING: 'ligando',
    READY: 'online',
 ...
 */
export function estreitarEstado(state) {
    if (state === undefined)
        return undefined;
    return ROTULOS_DE_ESTADO[state] === undefined
        ? undefined
        : state;
}
/* ========================================================================== */
 ...
 */
export function textoDeEstado(projecao, agora) {
    ...
    if (estado === 'READY') {
        if (projecao.url !== undefined)
            linhas.push(`Túnel: ${projecao.url}`);
        ...
        if (projecao.expiresAt !== undefined) {
            linhas.push(`Expira: em ${formatarDuracao(projecao.expiresAt - agora)} (${formatarHora(projecao.expiresAt)})`);
        }
    }
    return linhas.join('\n');
}
 ...
 */
export function textoDeEstadoCurto(projecao, agora) {
    const estado = estreitarEstado(projecao.state);
    switch (estado) {
        case 'READY': {
            const linhas = [`📶 Túnel *online* há ${formatarDuracao(agora - (projecao.readyDesde ?? agora))}.`];
            if (projecao.url !== undefined)
                linhas.push(`Link: ${projecao.url}`);
            if (projecao.expiresAt !== undefined) {
...
export function cortarTexto(texto, max = MAX_TEXTO_MENSAGEM) {
 ...
 */
export const ROTULOS_DE_STATUS_DE_AGENTE = Object.freeze({
    running: 'rodando',
    done: 'concluído',
    failed: 'falhou',
    cancelled: 'cancelado',
 ...
 * terminal no meio.
 */
export function sanearUmaLinha(texto) {
    let saida = '';
 ...
 */
export function haQuantoTempo(ms) {
    if (ms <= 0)
        return 'agora mesmo';
 ...
 */
export function linhaDeRun(run, agora) {
    ...
    if (run.kind !== undefined)
        extras.push(ROTULOS_DE_KIND_DE_RUN[run.kind]);
    if (run.worktree !== undefined)
        extras.push(`wt: ${run.worktree}`);
    extras.push(`📊 ${resumoDeMetricas(run.metrics)}`);
    const linha = `${base} · ${extras.join(' · ')}`;
    if (run.summary === undefined || run.summary.length === 0)
        return linha;
    return `${linha}\n   💬 ${run.summary}`;
}
/**
 ...
 */
export function textoDeRelatorioDeAgentes(runs, agora) {
    if (runs.length === 0)
        return 'Nenhum agente rodando.';
 ...
 */
export function textoDeFimDeRuns(runs, agora) {
 ...
 */
export const ROTULOS_DE_KIND_DE_RUN = Object.freeze({
    agent: 'agente',
    chat: 'chat',
    worktree: 'worktree',
});
 ...
 */
export function formatarTempoDeMs(ms) {
    if (ms <= 0)
        return '0 ms';
    if (ms < 1_000)
 ...
 */
export function totalDeTokens(metrics) {
    ...
    const presentes = contadores.filter((valor) => valor !== undefined);
 ...
 */
export function tempoDeTrabalhoMs(metrics) {
    if (metrics === undefined)
        return undefined;
    const presentes = [metrics.llmMs, metrics.toolMs].filter((valor) => valor !== undefined);
    if (presentes.length === 0)
        return undefined;
    return presentes.reduce((soma, valor) => soma + valor, 0);
}
 ...
 */
export function resumoDeMetricas(metrics) {
    const partes = [];
    const tokens = totalDeTokens(metrics);
    if (tokens !== undefined)
        partes.push(`${formatarQuantidade(tokens)} tokens`);
    const tempo = tempoDeTrabalhoMs(metrics);
    if (tempo !== undefined)
        partes.push(formatarTempoDeMs(tempo));
    return partes.length === 0 ? '—' : partes.join(' / ');
}
/** Um campo de metrica, ou «—» quando o host o nao mediu. */
function campoDeMetrica(valor, formatar) {
 ...
 */
function detalheDeMetricas(metrics) {
    const q = (valor) => campoDeMetrica(valor, formatarQuantidade);
    const t = (valor) => campoDeMetrica(valor, formatarTempoDeMs);
    return [
 ...
 */
export function textoDeTarefa(runs, id, agora) {
    const run = runs.slice(0, TETO_DE_RUNS_DO_RELATORIO).find((cada) => cada.id === id);
    if (run === undefined)
        return `Tarefa ${id} não encontrada (veja /agentes)`;
 ...
 * de ajuda que cresce.
 */
export const TEXTO_DE_AJUDA = 'ℹ️ Este bot controla o acesso ao teu Harness pelo Telegram.\n' +
