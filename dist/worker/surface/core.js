...
import { botoesDeAlerta, extrairAlerta } from "./actions.js";
import { cortarTexto, estreitarEstado, formatarDuracao, TEXTO_DE_AJUDA, textoDeEstadoCurto, textoDeFimDeRuns, textoDeRelatorioDeAgentes, textoDeTarefa, } from "./text.js";
import { criarOutbox } from "./outbox.js";
import { gerarRequestId, gerarTokenOpaque } from "./tokens.js";
import { COMANDOS_PUBLICADOS } from "./commands.js";
import { LIMITES_PAREAMENTO_PADRAO, RESPOSTA_AGUARDANDO_CANCELADO, RESPOSTA_AGUARDANDO_EXPIROU, RESPOSTA_PEDIR_VALOR, RESPOSTA_PEDIR_VALOR_MALFORMADO, } from "./auth.js";
...
export { COMANDOS_PUBLICADOS } from "./commands.js";
/** Publica a lista canonica. TG-080: o array chega inteiro, na ordem. */
export async function registarComandosPublicados(api) {
 ...
 */
export function extrairNomeDeComando(texto) {
...
function indiceDeEspacoAscii(valor) {
...
export function textoDeRecusa(codigo) {
    switch (codigo) {
...
export function criarProjecao() {
     ...
     */
    function marcarReadySe(estado, anterior, agora) {
        if (estado === 'READY' && anterior !== 'READY')
            readyDesde = agora;
        if (estado !== 'READY')
            readyDesde = undefined;
    }
    return {
        ler: () => ({ state, seq, url, expiresAt, readyDesde }),
        aplicarDifusao(msg, agora) {
            if (msg.seq <= seq)
                return false;
 ...
 */
export function runsTerminadosDesde(anterior, atual) {
    const anteriorPorId = new Map((anterior ?? []).map((run) => [run.id, run]));
 ...
 */
function textoDeResultadoDoAck(acao, result) {
    switch (acao) {
        case 'tunnel.up':
 ...
 * as recusas delas.
 */
function ehAcaoDeAgente(acao) {
    return (acao === 'agent.dispatch' ||
        acao === 'agent.status' ||
        acao === 'agent.cancel' ||
        acao === 'chat.new' ||
        acao === 'worktree.create');
}
export function criarNucleo(deps) {
     ...
     */
    function atrasoMalformado(falhasAnteriores) {
     ...
     */
    function armarEsperaDeValor(identity) {
        if (aguardandoValor.size >= MAX_AGUARDANDO && !aguardandoValor.has(identity.chatKey)) {
            ...
            if (maisAntigo !== undefined)
                aguardandoValor.delete(maisAntigo);
        }
     ...
     */
    const estadoOutbox = criarOutbox((chat, texto) => mostrarEstado(chat, texto).then(() => undefined), { maxLength: limites.maxTextLength, time, log });
    /**
     ...
     */
    async function mostrarEstadoSerializado(chat, texto) {
        const estado = porChat.get(chat) ?? { ultimoEnvio: Number.NEGATIVE_INFINITY, pendente: undefined, emEspera: false };
    ...
    const cartoesDoControle = new Map();
    /**
     ...
     */
    const confirmacaoNoCartao = new Map();
    /** O texto do cartao na linha de estado (Regra 3). So para o DONO. */
    function linhaDeEstadoDoCartao(agora) {
        ...
        if (estado === 'STOPPED')
            return 'Túnel: ⬜ Desligado — nada exposto';
     ...
     */
    async function exibirCartao(chat, agora) {
        ...
        if (confirmacaoNoCartao.has(chat))
            return registado;
     ...
     */
    function emSegundoPlano(chat, tarefa) {
     ...
     */
    function enviarIntent(intent, identidade, messageTarget) {
        const pedido = {
            ...intent,
     ...
     */
    function emitirLinkAutomatico(arma) {
        if (projecao.ler().state !== 'READY')
            return;
        enviarIntent({ intent: 'session.issue', requestId: gerarRequestId(time.now()) }, { userKey: arma.from, chatKey: arma.chat }, undefined);
    }
    /* ---- o despacho de comandos (port fiel de `tratarComando`) -------------- */
    async function tratarComando(nome, identidade, texto) {
        switch (nome) {
            case 'ligar':
    ...
    async function tratarAcao(event) {
        ...
        switch (event.action) {
            ...
            case 'secret.rotate': {
                ...
                if (vindaDoCartao && confirmacaoAtiva === event.action) {
                    ...
                    enviarIntent({
                        ...
                        requestId: requestIdConfirm,
                        nonce: event.token,
                    }, event.identity, cartoesDoControle.get(chat));
                    return;
                }
                ...
                if (vindaDoCartao) {
                    ...
                    if (event.action === 'tunnel.up')
                        await comandos.ligar(event.identity, cartao);
                    else
                        await comandos.rotacionar(event.identity, cartao);
                ...
                enviarIntent({
                    ...
                    requestId: requestIdUp,
                    nonce: event.token,
                }, event.identity, event.messageTarget);
                return;
            }
            case 'tunnel.down': {
                ...
                await comandos.confirmarDesligar(event.identity, event.token, event.answerTarget, event.messageTarget);
                return;
            }
            case 'session.issue': {
            ...
            case 'emergency':
                ...
                await deps.sender.answer(event.answerTarget, vindaDoCartao ? { text: 'A derrubar tudo…' } : undefined);
    ...
    function ehParearSemValor(texto) {
        const aparado = texto.trim();
        if (!aparado.startsWith('/'))
            return false;
        const espaco = indiceDeEspacoAscii(aparado);
    ...
    function eIntencaoDeCancelar(texto) {
     ...
     * o fluxo inline.
     */
    async function validarValorAguardado(identity, candidato) {
        ...
        if (veredito.kind !== 'recusado')
            return;
        if (veredito.delayMs > 0)
            await time.sleep(veredito.delayMs);
     ...
     */
    async function tratarDuranteEspera(identity, registo, texto) {
        ...
        if (nome !== undefined) {
            ...
            if (nome === 'parear' && ehParearSemValor(texto))
                return;
            aguardandoValor.delete(identity.chatKey);
        ...
        if (registo.userKey !== identity.userKey)
            return;
     ...
     */
    async function continuarComando(event) {
        ...
        if (pareamento.kind !== 'ignorado') {
            ...
            if (pareamento.delayMs > 0)
                await time.sleep(pareamento.delayMs);
    ...
    function onAck(msg) {
        try {
            ...
            if (pendente.acao === 'tunnel.status') {
                ...
                emSegundoPlano(pendente.chatKey, () => mostrarEstado(pendente.chatKey, textoDeEstadoCurto(projecao.ler(), time.now())));
                return;
            }
            ...
            if (pendente.acao === 'agent.cancel') {
                ...
                const texto = msg.result === 'accepted'
    ...
    function onPairingChallenge(msg) {
        try {
            // S3-b: o digest NUNCA sai deste ramo — nem em log, nem em payload.
            const digest = Buffer.from(msg.digest, 'hex');
            const desafio = {
                expiresAt: msg.expiresAt,
                verify: (candidato) => timingSafeEqual(createHash('sha256').update(candidato, 'utf8').digest(), digest),
            };
    ...
    function statusPendentes() {
        ...
        for (const p of pendentes.values()) {
            if (p.acao === 'agent.status')
                pedidos.push(p);
        }
        return pedidos;
    }
    function onAgentReport(msg) {
        try {
            ...
            if (pedidos.length > 0) {
                ...
                for (const pedido of pedidos) {
                    const texto = pedido.agentId === undefined
