 ...
 */
import { cortarTexto, MAX_BASE_CHARS, MAX_PROMPT_CHARS, sanearUmaLinha } from "./text.js";
 ...
 */
export const COMANDOS_PUBLICADOS = Object.freeze([
...
export function comandoPublicado() {
 ...
 * onda 5a).
 */
function emitirIntent(ctx, identidade, pedido) {
    const pedidoNeutro = {
 ...
 */
async function mostrarConfirmacao(ctx, chat, alvoDeEdicao, texto, actionRows) {
    const opcoes = { actionRows };
...
export function criarOnOff(ctx) {
    const tokens = new Map();
    function emitirTokenDeDesligar(userKey, chatKey) {
    ...
    return {
        async ligar(identidade, alvoDeEdicao) {
            ...
            await mostrarConfirmacao(ctx, identidade.chatKey, alvoDeEdicao, '🟢 Ligar o túnel agora? Quando abrir, o link de acesso chega aqui por si só.', [[acao, linhaDeCancelar()]]);
        },
        async desligar(identidade, alvoDeEdicao) {
            ...
            await mostrarConfirmacao(ctx, identidade.chatKey, alvoDeEdicao, '🔴 Desligar o túnel derruba o acesso remoto. Continuar?', [[acao, linhaDeCancelar()]]);
        },
        async confirmarDesligar(identidade, token, answerTarget, messageTarget) {
...
export function criarAccess(ctx) {
    return {
        async acessar(identidade) {
        ...
        async rotacionar(identidade, alvoDeEdicao) {
            ...
            await mostrarConfirmacao(ctx, identidade.chatKey, alvoDeEdicao, '⇄ Gerar chave nova invalida a atual e as sessões abertas. Continuar?', [[acao, linhaDeCancelar()]]);
        },
    };
}
export function criarStatus(ctx) {
    let emergenciaDisparada = false;
    return {
        async status(identidade) {
            // Leitura pura; o ack traz o estado autoritativo.
            const requestId = gerarRequestId(ctx.time.now());
            emitirIntent(ctx, identidade, { intent: 'tunnel.status', requestId });
        },
        async emergencia(identidade) {
...
function partirPrimeiroEResto(argumentos) {
    const espaco = indiceDeEspacoAsciiLocal(argumentos);
    if (espaco === -1)
        return { primeiro: argumentos, resto: '' };
...
function partirSkillEPrompt(argumentos) {
...
function indiceDeEspacoAsciiLocal(valor) {
...
export function criarAgentes(ctx) {
    /**
    ...
    return {
        async agente(identidade, argumentos) {
            ...
            await mostrarConfirmacao(ctx, identidade.chatKey, undefined, `🤖 Disparar o agente "${skill}"?\nEle executa código na tua máquina com este prompt:\n"${promptFinal}"`, [[acao, linhaDeCancelar()]]);
        },
        async agentes(identidade) {
        ...
        async pararAgente(identidade, argumentos) {
        ...
        async confirmarDispatch(identidade, token, answerTarget, messageTarget) {
            // TG-027: responder em TODOS os caminhos — inclusive no de recusa.
...
export function criarTarefas(ctx) {
    /**
     * O pedido em espera CHAVEADO pelo token OPACO do botao (o nonce do host).
     * NAO e validacao do nonce (S5): e a correlacao entre o clique e o pedido da
     ...
     */
    async function iniciarCriacao(identidade, tipo, params, texto) {
     ...
     */
    async function confirmarCriacao(tipo, identidade, token, answerTarget, messageTarget) {
        const registado = criacoes.get(token);
        const agora = ctx.time.now();
        const valido = registado !== undefined &&
            registado.tipo === tipo &&
    ...
    return {
        async novoChat(identidade, argumentos) {
            ...
            const promptFinal = cortarTexto(sanearUmaLinha(prompt), MAX_PROMPT_CHARS);
            await iniciarCriacao(identidade, 'chat.new', { prompt: promptFinal }, `💬 Iniciar um chat novo com este prompt?\nEle executa código na tua máquina:\n"${promptFinal}"`);
        },
        async novoChatWt(identidade, argumentos) {
            ...
            const promptFinal = cortarTexto(sanearUmaLinha(prompt), MAX_PROMPT_CHARS);
            await iniciarCriacao(identidade, 'chat.new', { prompt: promptFinal, worktree }, `💬 Iniciar um chat novo no worktree "${worktree}" com este prompt?\nEle executa código na tua máquina:\n"${promptFinal}"`);
        },
        async criarWorktree(identidade, argumentos) {
            ...
            await iniciarCriacao(identidade, 'worktree.create', { nome, ...(baseFinal === undefined ? {} : { base: baseFinal }) }, `📁 Criar o worktree "${nome}"${baseFinal === undefined ? '' : ` a partir de "${baseFinal}"`}?\nEle cria uma pasta nova na tua máquina.`);
        },
        async statusTarefa(identidade, argumentos) {
            const agentId = argumentos.trim();
            if (!GRAMMAR_DE_ID_DE_RUN.test(agentId)) {
        ...
        async confirmarChatNovo(identidade, token, answerTarget, messageTarget) {
            await confirmarCriacao('chat.new', identidade, token, answerTarget, messageTarget);
        },
        async confirmarWorktree(identidade, token, answerTarget, messageTarget) {
            await confirmarCriacao('worktree.create', identidade, token, answerTarget, messageTarget);
        },
    };
}
export function criarComandosDaSuperficie(ctx) {
    return {
...
export function criarComandosDeSuperficie(ctx) {
    ...
    return {
        ...
        confirmarDesligar: (identidade, token, answerTarget, messageTarget) => onoff.confirmarDesligar(identidade, token, answerTarget, messageTarget),
        ...
        confirmarDispatch: (identidade, token, answerTarget, messageTarget) => agentes.confirmarDispatch(identidade, token, answerTarget, messageTarget),
        ...
        confirmarChatNovo: (identidade, token, answerTarget, messageTarget) => tarefas.confirmarChatNovo(identidade, token, answerTarget, messageTarget),
        confirmarWorktree: (identidade, token, answerTarget, messageTarget) => tarefas.confirmarWorktree(identidade, token, answerTarget, messageTarget),
    };
}
