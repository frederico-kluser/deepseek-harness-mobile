 ...
 */
import { ProviderError, WORKER_EXIT } from "./interno.js";
 ...
 */
export function lerTokenDoAmbiente(env) {
    const cru = env[TOKEN_ENV_VAR];
    if (cru === undefined || cru.trim() === '') {
        throw new ProviderError(WORKER_EXIT.CONFIG, 'TOKEN_MISSING', `a variavel de ambiente ${TOKEN_ENV_VAR} esta ausente ou vazia; ` +
 ...
 */
export function assertTokenNotInArgv(argv, token) {
    for (const [index, arg] of argv.entries()) {
        const literal = token !== undefined && token.length >= 8 && arg.includes(token);
        if (literal || BOT_TOKEN_SHAPE.test(arg)) {
            throw new ProviderError(WORKER_EXIT.CONFIG, 'TOKEN_IN_ARGV', `argv[${index}] contem algo com forma de token do Telegram. ` +
