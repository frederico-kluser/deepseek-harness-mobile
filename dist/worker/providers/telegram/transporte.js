 ...
 */
import { describeForLog, systemTime } from "./interno.js";
/* ========================================================================== */
/* Auto-retry do 429                                                          */
/* ========================================================================== */
 ...
 */
export const METODOS_SEM_RETRY = ['getUpdates'];
/**
...
export function retryAfterSeconds(response) {
    if (response.ok)
        return undefined;
    if (response.error_code !== TOO_MANY_REQUESTS)
        return undefined;
...
export function createAutoRetryTransformer(options = {}) {
    ...
    return async (prev, method, payload, signal) => {
        ...
        for (;;) {
            ...
            if (METODOS_SEM_RETRY.includes(method))
                return response;
...
export function isSamplePoint(consecutive) {
    return consecutive > 0 && (consecutive & (consecutive - 1)) === 0;
}
export function createTransportLogTransformer(options) {
    ...
    return async (prev, method, payload, signal) => {
        try {
            ...
            if (isSamplePoint(consecutive)) {
                const mensagem = 'o pedido HTTP a Bot API falhou (rede). O bot continua vivo e volta a tentar.';
                const campos = {
                    method,
                    falhas_seguidas: consecutive,
                ...
                if (consecutive >= ESCALATE_AFTER)
                    options.log.error(mensagem, campos);
                else
                    options.log.warn(mensagem, campos);
            }
