...
import { ProviderError, exitCodeFor, describeForLog, WORKER_EXIT, } from "./interno.js";
 ...
 */
export function assertPollingOptions(options) {
    if (options.allowed_updates === undefined) {
        throw new ProviderError(WORKER_EXIT.POLLING, 'POLLING_FAILED', 'allowed_updates omitido: o servidor MANTERIA a configuracao anterior, ' +
            'que e estado invisivel guardado do lado do Telegram.');
    }
    if (options.allowed_updates.length === 0) {
        throw new ProviderError(WORKER_EXIT.POLLING, 'POLLING_FAILED', 'allowed_updates vazio: na Bot API isso NAO e "nenhum", e o reset para o ' +
    ...
    if (options.timeout === undefined || options.timeout > LONG_POLL_MAX_TIMEOUT) {
        throw new ProviderError(WORKER_EXIT.POLLING, 'POLLING_FAILED', `timeout tem de existir e ser <= ${LONG_POLL_MAX_TIMEOUT} (o servidor clampa la de qualquer forma).`);
    }
    if (options.drop_pending_updates !== true) {
        throw new ProviderError(WORKER_EXIT.POLLING, 'POLLING_FAILED', 'drop_pending_updates tem de ser true no boot: ate 24 h de comandos represados ' +
 ...
 */
export function classifyPollingError(error) {
    if (error instanceof GrammyError && error.error_code === CONFLICT_OTHER_GET_UPDATES) {
        return {
            ...
            message: 'CONFLITO 409: outro getUpdates esta a correr com este mesmo token. ' +
    ...
    if (error instanceof GrammyError && error.error_code === UNAUTHORIZED) {
        return {
            ...
            message: 'NAO AUTORIZADO 401: o token do bot foi recusado. Foi revogado, ou a ' +
...
export async function runPolling(deps) {
    ...
    try {
        ...
        const resultado = await Promise.race([started.then(() => 'stopped'), prazo]);
        if (resultado === 'boot-timeout') {
            const code = 'BOOT_TIMEOUT';
            const erro = new ProviderError(WORKER_EXIT.BOOT_TIMEOUT, code, `o arranque nao chegou a receber updates em ${bootTimeoutMs} ms`);
            deps.log.error('ARRANQUE ENCRAVADO: passaram ' +
                ...
                'supervisor do host aplique a sua propria politica de reinicio.', { code: erro.reason, exit_code: erro.code, boot_timeout_ms: bootTimeoutMs });
...
async function pararComCuidado(deps) {
    try {
        await deps.bot.stop();
    }
    catch (error) {
