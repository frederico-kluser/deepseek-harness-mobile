 ...
 */
import { ProviderError, exitCodeFor } from "./interno.js";
/* ========================================================================== */
 ...
 */
export const INCREASES_EXPOSURE = Object.freeze({
    'tunnel.up': true,
    'tunnel.down': false,
    'tunnel.status': false,
    'session.issue': true,
 ...
 */
export function buildCallbackData(action, token) {
    if (token.length === 0) {
    ...
    if (!TOKEN_ALPHABET.test(token)) {
        throw new Error(`token fora do alfabeto base64url para a accao ${action}: um separador dentro do token partiria o parser`);
    }
    ...
    if (bytes > CALLBACK_DATA_MAX_BYTES) {
        ...
        throw new ProviderError(exitCodeFor('CALLBACK_DATA_TOO_LONG'), 'CALLBACK_DATA_TOO_LONG', `callback_data com ${bytes} bytes (limite ${CALLBACK_DATA_MAX_BYTES}); ` +
...
export function parseCallbackData(data) {
    ...
    if (parts.length !== 3)
        return { ok: false, reason: 'deny:callback-data-unknown-schema' };
    const [schema, action, token] = parts;
    if (schema !== CALLBACK_SCHEMA)
        return { ok: false, reason: 'deny:callback-data-unknown-schema' };
    ...
    if (!TOKEN_ALPHABET.test(token))
        return { ok: false, reason: 'deny:callback-data-malformed-token' };
    return { ok: true, action: action, token };
}
...
function detectSurface(update) {
    ...
    if (isObject(update.channel_post))
        return 'channel_post';
    if (isObject(update.edited_channel_post))
        return 'edited_channel_post';
    if (isObject(update.callback_query))
        return 'callback_query';
    if (isObject(update.inline_query))
        return 'inline_query';
    if (isObject(update.my_chat_member))
        return 'my_chat_member';
    if (isObject(update.chat_member))
        return 'chat_member';
    return 'unknown';
}
/**
...
function readId(objeto) {
    if (!isObject(objeto))
        return undefined;
    return objeto.id;
}
/** Lê o `chat` de uma mensagem crua (ou `undefined`). */
function readChatDe(msg) {
    if (!isObject(msg))
        return undefined;
    return msg.chat;
}
function finish(rawFrom, rawChat, surface) {
...
function toSurfaceIdentity(extraida) {
...
function readCqId(cq) {
    if (!isObject(cq))
        return undefined;
    const id = cq.id;
    return typeof id === 'string' && id.length > 0 ? id : undefined;
}
function readMessageId(cq) {
    if (!isObject(cq))
        return undefined;
