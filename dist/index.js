...
import { registerSessaoNovaObserver } from "./audit/events.js";
import { resolveAuditLogPath } from "./audit/log.js";
import { criarCoalescedor, criarObservadorSessaoNova, criarRelatorioPeriodico } from "./audit/notify.js";
import { assertValidConfig } from "./config/assert.js";
import { assertSecureBind } from "./config/bind.js";
import { resolveControl, resolveExposure, shouldAutoStartTunnel } from "./config/schema.js";
import { mayTrustEdgeClientIp } from "./config/schema.js";
import { IPC_PROTOCOL_VERSION } from "./contracts/ipc.js";
import { createConfirmService } from "./control/confirm.js";
import { createTunnelController, ORIGEM_BOOT } from "./control/controller.js";
import { criarRespondedorDeNonce, criarRespondedorIpc } from "./control/surface-ipc.js";
import { createAgentRegistry, DEFAULT_PROVIDER_NAME, } from "./agents/registry.js";
import { criarServicoDeChats } from "./agents/chats.js";
import { criarServicoDeWorktrees } from "./agents/worktrees.js";
import { criarColetorDeMetricas } from "./agents/metrics.js";
import { resolveAgents } from "./config/schema.js";
import { PLUGIN_NAME } from "./errors.js";
import { createTunnelProxy } from "./tunnel/proxy.js";
import { createGateAuthStack, createRequestOriginResolver, createTunnelOriginRegistry, } from "./http/session-auth.js";
import { createCsrfGuard } from "./panel/csrf.js";
import { createPanelRouter, PANEL_PREFIX } from "./panel/routes.js";
import { createOneTimeTokenStore } from "./secret/ott.js";
import { createMagicStore } from "./session/magic.js";
import { createNativeUiSurface, UI_REQUESTED_BY } from "./ui-contrib/surface.js";
import { criarSessaoDePareamento } from "./telegram/pairing.js";
import { derivarEstadoDoBot } from "./ui-contrib/bot-state.js";
import { analisarSecretsEnv, caminhoDoSecretsEnv, gravarSecretsEnv, lerSecretsEnv, validarFormatoDe, } from "./telegram/onboarding.js";
import { apiRootDe, criarSonda } from "./onboarding/sonda.js";
import { arrivedViaTunnel } from "./http/host-header.js";
import { normalizeRemoteAddress } from "./http/origin.js";
import { EDGE_CLIENT_IP_HEADER } from "./http/session-auth.js";
import { DEFAULT_PROVIDER, PROVIDER_ENV, } from "./proc/env.js";
/**
 ...
 */
export { createRequestOriginResolver } from "./http/session-auth.js";
import { readSessionCookie, assertTrustworthyOrigin, serializeSessionCookie } from "./session/cookie.js";
import { createLinkTokenStore } from "./session/link-token.js";
import { createGuardLogger } from "./logging/logger.js";
import { requestsDeniedPermission } from "./permissions/deny.js";
import { createWorkerSupervisor, defaultSupervisorDeps, } from "./proc/supervisor.js";
import { createStateStore } from "./state/store.js";
import { resolveStatePaths } from "./state/paths.js";
import { createTunnelDiscovery } from "./tunnel/discover.js";
import { defaultOrphanSweepDeps, EVENTO_ORFAO, ownerOrphanMessage, recoverTunnelAtBoot, sweepOrphanTunnel, } from "./tunnel/pidfile.js";
import { createHttpProbeTransport } from "./tunnel/probe.js";
import { createTunnelReadiness } from "./tunnel/readiness.js";
import { createTunnelSupervisor } from "./tunnel/supervisor.js";
import { createTtlEffects } from "./tunnel/ttl.js";
export { PLUGIN_NAME } from "./errors.js";
export { shouldAutoStartTunnel, resolveExposure } from "./config/schema.js";
/**
 ...
 */
export const UNAUTHENTICATED_PANEL_PREFIXES = [
    '/__guard/magic',
    '/__guard/secret',
    '/__guard/api/login',
];
 ...
 * partia o produto.
 */
export const LOOPBACK_ONLY_PREFIXES = ['/__guard/secret'];
/* ========================================================================= */
 ...
 */
function criarAlocadorDeMetricas(log) {
    let reserva;
    let fechado = false;
    const reservar = () => {
        const servidor = createNetServer();
        servidor.once('error', (error) => {
            log.error(`nao foi possivel reservar a porta de metricas: ${error instanceof Error ? error.message : String(error)}`);
        });
 ...
 * aviso.
 */
function recuperarBoot(config, log, store, efeitos, 
...
sweep) {
    ...
    const effects = createTtlEffects({
        ...
        notifyOwner: (message) => efeitos.notificarDono(message),
    });
    recoverTunnelAtBoot({
 ...
 */
export function criarFanoutDeEstado(lerAtual, log) {
    const observadores = [];
    return {
        assinar(listener) {
            try {
                ...
                log.warn(`observador de estado falhou no replay imediato: ${error instanceof Error ? error.message : String(error)}`);
            }
...
export function apply(ctx, config, options = {}) {
    ...
    if (config.encodedAuthString === undefined) {
        ...
        log.warn('config.encodedAuthString AUSENTE: nenhuma credencial estatica esta configurada, ' +
    ...
    if (config.trustedRemotes.length === 0) {
        ...
        log.warn('config.trustedRemotes esta vazio: politica fail-closed ativa, ' +
    ...
    if (config.guardedPrefixes.length === 0) {
        ...
        // desse inventario.
        log.warn('config.guardedPrefixes esta VAZIO: o inventario do plano de controlo nao esta ' +
    ...
    if (config.deniedPermissions.length === 0) {
        ...
        log.warn('config.deniedPermissions esta VAZIO: NENHUMA elevacao de permissao sera vetada, ' +
    ...
    if (config.exposure === undefined) {
        ...
        log.warn('config.exposure AUSENTE: assume-se a leitura mais fechada -- ' +
    ...
    if (config.agents === undefined) {
        ...
        log.warn('config.agents AUSENTE: assume-se a leitura mais fechada -- skills vazio ' +
    ...
    if (exposure.trustEdgeHeaders) {
        ...
        log.warn(`config.exposure.trustEdgeHeaders=true: o cabecalho da borda passa a decidir ` +
     ...
     */
    log.info('ORDEM DE CARREGAMENTO: sem exigencia. A barreira e dona do despacho do ' +
    ...
    log.info(`Portao ativo em ${ctx.webServer.host}:${ctx.webServer.port} ` +
     ...
     */
    const provider = config.worker.provider ?? DEFAULT_PROVIDER;
    ...
    if (exposure.mode === 'tunnel' && exposure.autoStart) {
        log.info('exposure.autoStart=true: o tunel sobe no arranque, EXCETO se o modo restrito ' +
     ...
     */
    ctx.effect(() => ctx.on('security/permission-elevate', async (command, next) => {
        const denied = requestsDeniedPermission(command, config.deniedPermissions);
        if (denied !== undefined) {
            log.error(`VETO de elevacao de permissao: o comando pediu '${denied}', ` +
     ...
     */
    ctx.effect(() => ctx.on('http/auth-check', async (req, next) => {
        const temAutorizacao = typeof req.headers.authorization === 'string';
        const temSessao = readSessionCookie(typeof req.headers.cookie === 'string' ? req.headers.cookie : undefined) !== null;
        if (!temAutorizacao && !temSessao)
     ...
     */
    const capturarRegistroDeAcesso = (req) => {
        ...
        const ip = typeof rawEdge === 'string' && !rawEdge.includes(',') ? normalizeRemoteAddress(rawEdge) : undefined;
     ...
     */
    const issueSessionDoPortao = (req, presentedId) => {
        ...
        try {
            ...
            log.warn('nao emite sessao pelo link: origem nao entrega cookie Secure: ' +
                `${origem.scheme}://${host}`);
            return null;
        }
    ...
    const difundirNotificacao = (message) => {
    ...
    const fanoutDeEstado = criarFanoutDeEstado(() => {
        ...
        return atual === undefined
            ? { seq: 0, snapshot: { state: 'STOPPED', attempts: 0 } }
            : { seq: atual.seq, snapshot: atual };
    }, log);
    const broadcastControlador = (difusao) => {
        difundir(difusao);
    ...
    const pareado = (from, chat) => {
        try {
            ...
            log.error('nao foi possivel ler o pareamento persistido; intencao recusada: ' +
    ...
    ctx.effect(() => {
        ...
        if (config.tunnel === undefined) {
            ...
            log.info('controlador sem supervisor: nao ha configuracao de tunel (exposure.mode nao e tunnel); ' +
        ...
        const supervisor = createTunnelSupervisor({
            ...
            resolveOrigin: () => tunnelProxy.server,
            ...
            probe: {
                ...
                newCanaryToken: () => randomBytes(12).toString('hex'),
            },
        ...
        const tokenOps = (() => {
            ...
            return {
                ...
                validarFormato: (bruto) => validarFormatoDe(provider, bruto),
                fonte: fonteEfetiva,
                sondar: async (token) => {
                    ...
                    if (!prova.ok || prova.botNome === undefined)
                ...
                gravar: (token, handle) => {
                    ...
                    //    handle lembrado).
                    gravarSecretsEnv({ paths: statePaths, caminhoSecrets: caminhoDoSecretsEnv(statePaths) }, tokenVarDoProvedor, token);
                ...
                privacidade: async (forcar = false) => {
                    ...
                    const calcular = async () => {
                        if (token === undefined)
                            return { ok: true, handle: null, fonte: 'nenhum' };
                        const prova = await sonda.verificar(token);
                        ...
                        // inventa estado).
                        if (prova.ok)
                            return { ok: true, handle: prova.botNome ?? null, fonte };
            ...
            pairOps: (() => {
                ...
                return {
                    estado: () => {
                        ...
                        const base = { pareado: pareadoAtual };
                    ...
                    gerar: async () => {
                        ...
                        try {
                            if (authStack().state.read().pairing !== undefined) {
                                return { ok: false, erro: 'ja-pareado' };
                            }
                        }
                        catch (error) {
                        ...
                        if (workerSup === undefined) {
                            return { ok: false, erro: 'worker-indisponivel' };
                        }
                        ...
                        sessaoAtiva = sessao;
                        return { ok: true, codigo, expiraEm };
                    },
                };
            })(),
        ...
        const desregistrarObservador = registerSessaoNovaObserver((evento) => {
            ...
            if (!coalescedor.tentar('sessao-nova'))
                return;
    ...
    const sessoesDoHarness = () => servicoDoHarness('sessions');
    const controladorDeSessao = () => servicoDoHarness('sessionController');
    const projecoesDoHarness = () => servicoDoHarness('sessionProjections');
    /**
     ...
     */
    const baseDirDoHarness = () => {
        try {
            const raiz = ctx.agents?.roots()[0];
     ...
     */
    const difundirRelatorioDeAgentes = () => {
        ...
        const supervisor = createWorkerSupervisor(ctx, config, defaultSupervisorDeps, {
            ...
            onPairingSuccess: (msg) => {
                try {
                    ...
                    log.error(`nao foi possivel gravar o dono pareado no state.json: ${error instanceof Error ? error.message : String(error)}`);
                    return {
                        v: IPC_PROTOCOL_VERSION,
                        type: 'error',
                        code: 'INTERNAL',
