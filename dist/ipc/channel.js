...
const RESULTS = ['accepted', 'noop', 'rejected'];
const ERROR_CODES = [
...
function isCleanText(value, maxChars) {
 ...
 */
function isDisplayText(value, maxChars) {
...
/** Ids de identidade (`from`/`chat`) — STRING em V2 (EMENDA ONDA-1-IPC-ENVELOPE-STRING). A politica minima espelha o `normalizeKey` do worker (`worker/surface/ids.ts`): trim + nao vazio. NAO se valida FORMATO de provedor nenhum (um id nao-numerico nao e `[0-9]+` curto, um id de Matrix e uma url) — so se exige uma string utilizavel. A guarda de controlo impede ruido de terminal; o teto impede uma linha gigante. */
function isId(value) {
    return (typeof value === 'string' &&
        value.trim() !== '' &&
 ...
 * (worktree.create) e o campo `worktree` de `AgentRunReport`.
 */
 ...
 */
export function validateIpcMessage(value, direction) {
    ...
    if (bag['v'] !== IPC_PROTOCOL_VERSION)
        return fail('versao-desconhecida');
    const type = bag['type'];
    if (typeof type !== 'string')
        return fail('tipo-desconhecido');
    /**
...
function buildIntent(bag) {
    const { intent, requestId, from, chat, nonce, params } = bag;
    if (!isMember(INTENTS, intent))
        return fail('forma-invalida');
    if (!isCleanText(requestId, MAX_ID_CHARS))
        return fail('forma-invalida');
    if (!isId(from) || !isId(chat))
        return fail('forma-invalida');
    if (nonce !== undefined && !isCleanText(nonce, MAX_NONCE_CHARS))
        return fail('forma-invalida');
 ...
 * valida e RECONSTROI o payload `params`. PRESENTE SSE A INTENT O EXIGE (o
 * contrato):
 *
 ...
 */
function paramsAgentes(intent, params) {
    const exige = intent === 'agent.dispatch' ||
        intent === 'agent.cancel' ||
        intent === 'chat.new' ||
        intent === 'worktree.create';
    if (params === undefined)
        return exige ? null : undefined;
    ...
    if (intent === 'agent.dispatch') {
        ...
        if (!isSkillName(skill))
            return null;
        if (!isCleanText(prompt, MAX_MESSAGE_CHARS))
            return null;
        return { skill, prompt };
    }
    if (intent === 'agent.cancel') {
        const agentId = bag['agentId'];
        if (!isCleanText(agentId, MAX_ID_CHARS))
            return null;
        return { agentId };
    }
    if (intent === 'chat.new') {
    ...
    if (intent === 'worktree.create') {
        ...
        const nome = bag['nome'];
        if (!isWorktreeName(nome))
            return null;
        const base = bag['base'];
        if (base === undefined)
            return { nome };
        if (!isCleanText(base, MAX_WORKTREE_BASE_CHARS))
            return null;
        return { nome, base };
    }
...
function buildState(bag) {
    const { state, seq, url, expiresAt } = bag;
    if (!isMember(STATES, state))
        return fail('forma-invalida');
    ...
    if (!isCleanText(url, MAX_URL_CHARS) || !url.startsWith('https://'))
        return fail('forma-invalida');
...
function buildAck(bag) {
    const { requestId, result, state, code } = bag;
    if (!isCleanText(requestId, MAX_ID_CHARS))
        return fail('forma-invalida');
    if (!isMember(RESULTS, result))
        return fail('forma-invalida');
    if (!isMember(STATES, state))
        return fail('forma-invalida');
...
function buildError(bag) {
    const { requestId, code, message: text } = bag;
    if (!isMember(ERROR_CODES, code))
        return fail('forma-invalida');
    if (!isDisplayText(text, MAX_MESSAGE_CHARS))
        return fail('forma-invalida');
 ...
 */
function buildNotify(bag) {
    const { texto } = bag;
    if (!isDisplayText(texto, MAX_MESSAGE_CHARS))
        return fail('forma-invalida');
    const message = {
        v: IPC_PROTOCOL_VERSION,
        type: 'notify',
 ...
 */
function buildPairingChallenge(bag) {
 ...
 */
function buildNonceRequest(bag) {
    const { acao, requestId } = bag;
    if (!isMember(ACTIONS, acao))
        return fail('forma-invalida');
    if (!isCleanText(requestId, MAX_ID_CHARS))
        return fail('forma-invalida');
    const message = {
 ...
 */
function buildNonceIssued(bag) {
    const { acao, requestId, nonce, expiresAt } = bag;
    if (!isMember(ACTIONS, acao))
        return fail('forma-invalida');
    if (!isCleanText(requestId, MAX_ID_CHARS))
        return fail('forma-invalida');
    if (!isCleanText(nonce, MAX_NONCE_CHARS))
        return fail('forma-invalida');
    if (!isFiniteNumber(expiresAt))
        return fail('forma-invalida');
    const message = {
 ...
 */
function buildPairingOwner(bag) {
    const { from, chat, pairedAt } = bag;
    if (!isId(from) || !isId(chat))
        return fail('forma-invalida');
    if (!isFiniteNumber(pairedAt))
        return fail('forma-invalida');
    const message = {
        v: IPC_PROTOCOL_VERSION,
        type: 'pairing.owner',
 ...
 */
function buildPairingSuccess(bag) {
    const { from, chat, pairedAt } = bag;
    if (!isId(from) || !isId(chat))
        return fail('forma-invalida');
    if (!isFiniteNumber(pairedAt))
...
const RUN_KINDS = ['agent', 'chat', 'worktree'];
/**
 * EMENDA ONDA-2-CONTRATO-CAPACIDADES: as chaves do bloco `metrics`, na ORDEM
 * EXATA da reconstrucao — os DOIS codecs (host e worker) usam esta ordem, e o
 * gate de paridade (`test/unit/contracts/ipc.test.ts`) compara as linhas
 * byte a byte.
 */
const CHAVES_DE_METRICAS = [
    'inputTokens',
    'outputTokens',
    'cacheReadTokens',
    'cacheWriteTokens',
 ...
 */
function buildAgentMetrics(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return null;
    const bag = value;
    const out = {};
...
function buildAgentRun(bag) {
    ...
    if (!isFiniteNumber(startedAt))
        return null;
    if (summary !== undefined && !isDisplayText(summary, MAX_RUN_SUMMARY_CHARS))
        return null;
    ...
    if (kind !== undefined && !isMember(RUN_KINDS, kind))
        return null;
    if (worktree !== undefined && !isWorktreeName(worktree))
        return null;
 ...
 */
function buildAgentReport(bag) {
    const { runs } = bag;
    if (!Array.isArray(runs) || runs.length > MAX_RUNS_PER_REPORT)
        return fail('forma-invalida');
    const reconstruidos = [];
    for (const entrada of runs) {
        ...
        const run = buildAgentRun(entrada);
        if (run === null)
            return fail('forma-invalida');
 ...
 */
export function parseIpcLine(line, direction) {
    let parsed;
    try {
 ...
 * vez de o assumir.
 */
export function serializeIpcMessage(message, direction) {
    const verdict = validateIpcMessage(message, direction);
    if (!verdict.ok) {
        const tipo = String(message.type);
        throw new IpcChannelError('IPC_MESSAGE_INVALID', `mensagem '${tipo}' recusada pelo contrato (${verdict.reason}).`);
    }
    let line;
    try {
        ...
        throw new IpcChannelError('IPC_SERIALIZE_FAILED', error instanceof Error ? error.message : String(error));
    }
    if (line.includes('\n') || line.includes('\r')) {
 ...
 */
function responderSemNonce(pedido) {
    return {
        v: IPC_PROTOCOL_VERSION,
        type: 'error',
 ...
 */
export function createHostIpcChannel(options) {
    ...
    const limpar = (texto) => redact(texto, secrets());
    const descrever = (error) => limpar(error instanceof Error ? error.message : String(error));
    if (output === undefined) {
        ...
        log.error('Canal IPC SEM sentido host->worker: o filho nao tem `stdin`. ' +
            "Verifique que o `stdio` do spec declara `stdin: 'pipe'`.");
    }
    const porEscrever = () => output?.writableLength ?? 0;
     ...
     */
    const declararInviavel = () => {
        if (overwhelmed)
            return;
        overwhelmed = true;
        log.error(`Canal IPC INVIAVEL: ${String(porEscrever())} bytes por escrever ultrapassam o teto ` +
            `duro de ${String(overwhelmedBytes)}. O worker nao le ha demasiado tempo; ` +
            'nenhuma mensagem sera entregue ate ele ser reiniciado.');
    };
    const armarDreno = () => {
        if (drenoArmado || output === undefined)
            return;
        drenoArmado = true;
        output.once('drain', () => {
            drenoArmado = false;
    ...
    const esvaziar = () => {
        if (difusaoPendente === undefined || disposed || overwhelmed)
            return;
        if (output === undefined || !output.writable)
            return;
        ...
        if (saturado) {
            ...
            log.info(`Canal IPC drenou; ${String(coalesced)} difusoes de estado foram coalescidas ` +
    ...
    const send = (message) => {
        ...
        if (porEscrever() > maxPendingBytes) {
            if (!saturado) {
                ...
                log.warn(`Canal IPC saturado (${String(porEscrever())} bytes por escrever): o worker parou ` +
                    'de ler. As difusoes de estado passam a COALESCER; `ack` e `error` continuam a sair.');
            }
            if (difusaoPendente !== undefined)
                coalesced += 1;
        ...
        for (const verdict of decoder.push(chunk)) {
            ...
            try {
                if (intent !== undefined) {
                    ...
                    reply = (onPairingSuccess ?? responderSemPairing)(message);
                }
                else {
