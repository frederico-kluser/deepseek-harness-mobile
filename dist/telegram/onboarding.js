 ...
 */
import { closeSync, constants, fchmodSync, fsyncSync, fstatSync, openSync, readFileSync, renameSync, unlinkSync, writeSync, } from 'node:fs';
import { randomBytes } from 'node:crypto';
import { homedir } from 'node:os';
import { join, sep } from 'node:path';
import { PLUGIN_NAME } from "../errors.js";
import { redact } from "../logging/redact.js";
import { EXPOSURE_MASK, STATE_FILE_MODE } from "../state/paths.js";
...
import { COMANDO_CLI, textoSemDono, textoSemToken, textoPronto, textoTokenInvalido, tituloSemToken, tituloTokenInvalido, TITULO_PRONTO, TITULO_SEM_DONO, } from "./texts.js";
export { AVISOS_ANTES_DO_TUNEL, COMANDO_CLI, textoSemDono, textoSemToken, textoPronto, textoTokenInvalido, tituloSemToken, tituloTokenInvalido, TITULO_PRONTO, TITULO_SEM_DONO, TITULO_SEM_TOKEN, TITULO_TOKEN_INVALIDO, } from "./texts.js";
 ...
 */
export function validarFormatoDoToken(bruto) {
    ...
    if (!token.includes(':'))
        return { valido: false, motivo: 'sem-dois-pontos' };
    const casamento = FORMA_DO_TOKEN.exec(token);
    if (casamento === null)
        return { valido: false, motivo: motivoDetalhado(token) };
...
function motivoDetalhado(token) {
    const corte = token.indexOf(':');
    const id = token.slice(0, corte);
    const segredo = token.slice(corte + 1);
    if (id.startsWith('0'))
        return 'id-comeca-por-zero';
    if (!/^\d+$/u.test(id))
        return 'id-nao-numerico';
    if (!/^[A-Za-z0-9_-]*$/u.test(segredo))
 ...
 */
export function detectarEstado(retrato) {
    if (retrato.token === undefined)
        return 'SEM_TOKEN';
    if (!retrato.token.formato.valido)
        return 'TOKEN_INVALIDO';
    if (retrato.getMe === undefined || !retrato.getMe.ok)
        return 'TOKEN_INVALIDO';
    if (retrato.dono === undefined)
        return 'TOKEN_OK_SEM_DONO';
    return 'PRONTO';
}
/**
 ...
 */
export function proximoPasso(retrato, opcoes) {
...
export function analisarArgumentos(argv) {
    ...
    for (const bruto of argv) {
        ...
        if (nome === '--token' || nome === '-t' || PARECE_TOKEN.test(argumento)) {
            throw new OnboardingError('SETUP_TOKEN_IN_ARGV', 'a chave do bot não pode vir na linha de comando. Tudo o que se escreve ' +
        ...
        switch (nome) {
            ...
            case '--yes':
                ...
                break;
            default:
                throw new OnboardingError('SETUP_UNKNOWN_ARGUMENT', `não conheço a opção ${argumento}. Use \`${COMANDO_CLI} --ajuda\` para ver as que existem.`);
        }
    }
 ...
 */
export function analisarSecretsEnv(texto) {
 ...
 */
export function fundirSecretsEnv(existente, chave, valor) {
    if (!CHAVE_DE_AMBIENTE.test(chave)) {
        throw new OnboardingError('SECRETS_KEY_INVALID', `${JSON.stringify(chave)} não é um nome de variável de ambiente válido.`);
    }
    ...
    for (let i = linhas.length - 1; i >= 0; i -= 1) {
        ...
        if (linha === undefined || linha.trim().startsWith('#'))
            continue;
        const casamento = LINHA_DE_AMBIENTE.exec(linha);
        if (casamento?.[1] !== chave)
            continue;
        linhas[i] = nova;
        return linhas.join('\n');
    }
...
export function caminhoDoSecretsEnv(paths) {
 ...
 */
export function lerSecretsEnv(caminho) {
    let fd;
    try {
        ...
        if ((modo & EXPOSURE_MASK) !== 0) {
            throw new OnboardingError('SECRETS_MODE_TOO_OPEN', `o ficheiro ${NOME_DO_SECRETS_ENV} está legível por outras contas desta máquina ` +
        ...
        if (erro instanceof OnboardingError)
            throw erro;
        ...
        if (codigo === 'ENOENT' || codigo === 'ENOTDIR')
            return undefined;
        if (codigo === 'ELOOP') {
            throw new OnboardingError('SECRETS_READ_FAILED', `${NOME_DO_SECRETS_ENV} é um link simbólico. Por um link, quem controla o alvo ` +
                'escolhe de onde se lê a chave do bot; recusa-se.');
        }
        throw new OnboardingError('SECRETS_READ_FAILED', `não foi possível ler o ficheiro ${NOME_DO_SECRETS_ENV}: ${redact(mensagemDe(erro))}`);
    }
    finally {
        if (fd !== undefined)
            closeSync(fd);
 ...
 */
export function gravarSecretsEnv(ctx, chave, valor) {
    const existente = lerSecretsEnv(ctx.caminhoSecrets) ?? '';
    const conteudo = fundirSecretsEnv(existente, chave, valor);
    const tmp = join(ctx.paths.dir, `.${NOME_DO_SECRETS_ENV}.tmp-${process.pid.toString(36)}-${randomBytes(6).toString('hex')}`);
    try {
        const fd = openSync(tmp, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, MODO_DO_SECRETS_ENV);
        try {
        ...
        throw new OnboardingError('SECRETS_WRITE_FAILED', `não foi possível gravar o ficheiro ${NOME_DO_SECRETS_ENV}: ` +
 ...
 */
export function resolverToken(caminho, ambiente = process.env, chave = CHAVE_DO_TOKEN) {
 ...
 */
export function caminhoApresentavel(caminho, casa = homedir()) {
...
export { API_ROOT_PADRAO, classificarFalha, criarSondaHttp, lerIdentidade, LIMITE_DE_UPDATES, TIMEOUT_DA_SONDA_MS, } from "../onboarding/sonda.js";
/* ========================================================================== */
 ...
 */
export function validarFormatoDe(_provedor, bruto) {
