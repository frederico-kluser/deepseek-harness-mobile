...
function isOwnedByBarrier(listener) {
    return listener?.[BARRIER_OWNER_MARK] === true;
}
function markAsOwned(listener) {
 ...
 */
export function installAuthBarrier(server, wrappers, log) {
    const originalRequest = server.listeners('request');
    const originalUpgrade = server.listeners('upgrade');
    if (originalRequest.length === 0) {
        throw new GuardError('BARRIER_UNAVAILABLE', 'o node:http.Server nao tem listener `request` -- o WebServer ainda nao inicializou. ' +
    ...
    for (const listener of [...originalRequest, ...originalUpgrade]) {
        if (isOwnedByBarrier(listener)) {
            throw new GuardError('BARRIER_ALREADY_INSTALLED', 'ja existe uma barreira instalada neste node:http.Server. Empilhar duas e recusado ' +
    ...
    const failClosedHttp = (res, error) => {
        log.error('Falha nao tratada no despacho guardado; pedido recusado: ' +
    ...
    const guardedUpgrade = guardsUpgrade
        ...
        : undefined;
    const onUpgrade = guardedUpgrade === undefined
        ? undefined
        : (req, socket, head) => {
            let outcome;
            try {
    ...
    log.info(`Barreira instalada no despacho: request(${originalRequest.length} originais), ` +
    ...
    return () => {
        ...
        const stillOursUpgrade = onUpgrade === undefined || (nowUpgrade.length === 1 && nowUpgrade[0] === onUpgrade);
        server.removeListener('request', onRequest);
        if (onUpgrade !== undefined)
            server.removeListener('upgrade', onUpgrade);
        if (!stillOursRequest || !stillOursUpgrade) {
            const failure = new GuardError('BARRIER_OWNERSHIP_LOST', 'o despacho ja nao era nosso no momento da reversao ' +
