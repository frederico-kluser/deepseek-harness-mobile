...
function stripPort(value) {
    if (value.startsWith('[')) {
        ...
        if (end === -1)
            return undefined; // `[` sem fecho: forma invalida.
        const rest = value.slice(end + 1);
        if (rest.length > 0) {
            // Depois do `]` so pode vir `:<porta>`. `[::1]evil.com` nao e um `Host`.
            if (!rest.startsWith(':') || !PORT.test(rest.slice(1)))
                return undefined;
        }
    ...
    if (!PORT.test(value.slice(first + 1)))
        return undefined;
...
function pushGroups(parts, out) {
    for (const [index, part] of parts.entries()) {
        if (part.includes('.')) {
            ...
            if (octets.some((o) => !Number.isInteger(o) || o > 255))
                return false;
            out.push((((octets[0] ?? 0) << 8) | (octets[1] ?? 0)).toString(16), (((octets[2] ?? 0) << 8) | (octets[3] ?? 0)).toString(16));
            continue;
        }
...
function expandIpv6(value) {
    ...
    if (elision !== -1 && value.indexOf('::', elision + 2) !== -1)
        return undefined;
    let headText;
    let tailText;
    if (elision === -1) {
    ...
    const head = [];
    const tail = [];
    if (!pushGroups(headText.length === 0 ? [] : headText.split(':'), head))
        return undefined;
    if (!pushGroups(tailText.length === 0 ? [] : tailText.split(':'), tail))
        return undefined;
    if (elision === -1)
...
function collapseIpv6(groups) {
    const zeros = (upTo) => groups.slice(0, upTo).every((g) => g === '0');
    if (zeros(7) && groups[7] === '1')
        return '::1';
 ...
 */
export function canonicalRequestHost(rawHost) {
 ...
 */
export function buildAllowedRequestHosts(bindHost, tunnelOrigin) {
    const hosts = new Set();
 ...
 */
export function isAllowedRequestHost(rawHost, allowedHosts) {
    const host = canonicalRequestHost(rawHost);
    if (host === undefined)
        return false;
    if (isLocalhostName(host))
 ...
 */
export function arrivedViaTunnel(rawHost, tunnelOrigin) {
    const tunnel = hostOfOrigin(tunnelOrigin);
    if (tunnel === undefined)
        return false;
    const host = canonicalRequestHost(rawHost);
...
//# sourceMappingURL=host-header.js.map