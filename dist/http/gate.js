 ...
 * =============================================================================
 */
import { verifyBasicAuth } from "./auth-basic.js";
import { buildAllowedRequestHosts, canonicalRequestHost, isAllowedRequestHost, isLoopbackRequestHost, } from "./host-header.js";
import { isTrustedRemote } from "./origin.js";
import { isGuardedPath } from "./path.js";
import { denyNotFound, denyUnauthorized, denyUntrustedOrigin, denyUpgrade } from "./responses.js";
import { authenticateRequest, recordAudit, rewriteAuthenticatedTunnelRequest, } from "./session-auth.js";
import { emitSessaoNova } from "../audit/events.js";
 ...
 */
export function buildAllowedOrigins(loopbackAuthority, tunnelOrigin) {
 ...
 */
function refusedAtPerimeter(deps, req, surface) {
    const { config, log } = deps;
    if (!isTrustedRemote(req.socket.remoteAddress, config.trustedRemotes)) {
        log.warn(`[${surface}] Origem nao confiada recusada: ` +
    ...
    if (!isAllowedRequestHost(singleHeader(req, 'host'), allowedHosts)) {
        log.warn(`[${surface}] Host recusado (L2.5, anti DNS rebinding): ` +
 ...
 */
function extrairChaveDoQuery(raw) {
 ...
 */
export function createGuardedHandler(deps, delegate, surface) {
    const { ctx, log, config } = deps;
    /** Sessoes ja VISTAS por este handler (ponto de chamada PREP 5). */
    const sessoesVistas = new Map();
    return async (req, res) => {
        ...
        try {
            ...
            if (isGuardedPath(req.url, deps.loopbackOnlyPrefixes) &&
                !isLoopbackRequestHost(singleHeader(req, 'host'))) {
                log.warn(`[${surface}] 404 em ${String(req.url)}: rota de CANAL LOCAL APENAS ` +
             ...
             */
            authorized = await ctx.waterfall('http/auth-check', req, async () => outcome.authorized);
                ...
                if (!sessoesVistas.has(idHash)) {
                    if (sessoesVistas.size >= SESSAO_NOVA_TETO) {
                        ...
                        if (maisAntiga !== undefined)
                            sessoesVistas.delete(maisAntiga);
                    }
                    ...
                    if (!registado) {
                        log.warn(`[${surface}] sessao nova NAO registada (auditoria indisponivel); ` +
             ...
             */
            log.error(`[${surface}] falha no caminho de decisao; pedido NEGADO (fail-closed): ` +
        ...
        if (!authorized) {
            ...
            if (req.method === 'GET') {
                ...
                if (chave !== undefined &&
                    ...
                    if (setCookie !== null) {
                        ...
                        log.info(`[${surface}] chave no link VALIDA: 302 de ${destino} para a URL limpa com sessao`);
                        res.writeHead(302, {
                            Location: destino,
            ...
            log.warn(`[${surface}] 401 em ${String(req.method)} ${String(req.url)} ` +
        ...
        if (viaTunnel)
            rewriteAuthenticatedTunnelRequest(req, deps.loopbackAuthority);
        await delegate(req, res);
    };
}
/**
 ...
 */
export function createGuardedUpgradeHandler(deps, delegate, surface) {
    const { ctx, log, config } = deps;
    return async (req, socket, head) => {
        ...
        try {
            ...
            if (isGuardedPath(req.url, deps.loopbackOnlyPrefixes) &&
                !isLoopbackRequestHost(singleHeader(req, 'host'))) {
                log.warn(`[${surface}] Upgrade recusado em ${String(req.url)}: rota de CANAL LOCAL APENAS ` +
            ...
            if (rawOrigin !== undefined) {
                const allowedOrigins = buildAllowedOrigins(deps.loopbackAuthority, deps.tunnelOrigin.current());
                const origin = typeof rawOrigin === 'string' ? canonicalOrigin(rawOrigin) : undefined;
                if (origin === undefined || !allowedOrigins.includes(origin)) {
                    log.warn(`[${surface}] Upgrade recusado (CWE-1385, Origin fora da allowlist EXATA): ` +
            ...
            log.error(`[${surface}] Erro ao avaliar o upgrade; socket destruido: ` +
                `${error instanceof Error ? error.message : String(error)}`);
            socket.destroy();
            return;
        }
        if (!authorized) {
            log.warn(`[${surface}] Upgrade recusado (superficie do tunel sem sessao valida) em ` +
                `${String(req.url)}.`);
            denyUpgrade(socket, 401);
            return;
        }
        ...
        try {
            ...
            log.error(`[${surface}] o despacho original do upgrade rebentou; socket destruido: ` +
                `${error instanceof Error ? error.message : String(error)}`);
            socket.destroy();
        }
    };
