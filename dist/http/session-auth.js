...
import { mayTrustEdgeClientIp, resolveExposure } from "../config/schema.js";
import { AuditWriteError, openAuditLog } from "../audit/log.js";
import { DEFAULT_RATE_LIMIT_POLICY } from "../ratelimit/policy.js";
import { createRestrictedMode } from "../ratelimit/restricted.js";
import { createFailureTracker, runThrottledAttempt, } from "../ratelimit/tracker.js";
import { createSecretStore } from "../secret/store.js";
import { readSessionCookie } from "../session/cookie.js";
import { createSessionStore, systemClock } from "../session/store.js";
import { statePathsAt } from "../state/paths.js";
import { createStateStore } from "../state/store.js";
import { normalizeRemoteAddress } from "./origin.js";
import { arrivedViaTunnel } from "./host-header.js";
/* ========================================================================== */
 ...
 */
export const TRUSTED_EDGE_HEADERS = [EDGE_CLIENT_IP_HEADER, EDGE_FORWARDED_PROTO_HEADER];
export function createTunnelOriginRegistry() {
    let origin;
    return {
        current: () => origin,
        publish: (next) => {
 ...
 */
export function readPresentedSession(req, sessions) {
 ...
 */
export function presentsCredential(req) {
    if (typeof singleHeader(req, 'authorization') === 'string')
        return true;
 ...
 */
export function resolveRequestIdentity(req, input) {
    const identity = {};
    if (mayTrustEdgeClientIp(input.exposure, input.viaTunnel)) {
        ...
        if (normalized !== undefined)
            identity.ip = normalized;
    }
    if (input.session !== null)
        identity.sessionId = input.session.id;
    return identity;
}
/**
 ...
 */
export function presentedSecret(authorizationHeader) {
    if (typeof authorizationHeader !== 'string')
        return '';
    if (authorizationHeader.slice(0, 6).toLowerCase() !== 'basic ')
        return '';
 ...
 * ---------------------------------------------------------------------------
 */
export function recordAudit(auth, log, event) {
    try {
        ...
        if (error instanceof AuditWriteError) {
            log.error(`AUDITORIA INDISPONIVEL -- o portao NEGA (fail-closed). ${error.message} ` +
                `Ficheiro: ${error.path}`);
            return false;
        }
        log.error('AUDITORIA INDISPONIVEL -- o portao NEGA (fail-closed): ' +
...
export function verifyPresentedCredential(input) {
    ...
    try {
        ...
        input.log.error('SEGREDO INDISPONIVEL -- a leitura do estado falhou e a tentativa e NEGADA: ' +
            `${error instanceof Error ? error.message : String(error)}`);
...
const DENIED_WITHOUT_SESSION = (viaTunnel) => ({
    authorized: false,
    viaTunnel,
    session: null,
});
...
export async function authenticateRequest(input) {
     ...
     * apenas desejavel.
     */
    if (session === null && !presentsCredential(req))
    ...
    let outcome;
    try {
        outcome = await runThrottledAttempt(auth.limiter, identity, () => verifyPresentedCredential({
            req,
            config,
            auth,
            log,
        ...
        log.error('limitador de tentativas indisponivel; a tentativa e NEGADA: ' +
            `${error instanceof Error ? error.message : String(error)}`);
        return DENIED_WITHOUT_SESSION(viaTunnel);
    }
    /**
    ...
    try {
        ...
        if (intent !== undefined) {
            log.error('TETO DE FORCA BRUTA ATINGIDO: modo restrito ATIVO e exposicao derrubada ' +
        ...
        log.error('nao foi possivel persistir o modo restrito; a tentativa continua NEGADA: ' +
            `${error instanceof Error ? error.message : String(error)}`);
    }
 ...
 */
export function rewriteAuthenticatedTunnelRequest(req, loopbackAuthority) {
    req.headers.host = loopbackAuthority;
    for (const header of EDGE_FENCE_HEADERS)
 ...
 * sem portao".
 */
export function createGateAuthStack(options) {
    const clock = options.clock ?? systemClock;
    const policy = options.policy ?? DEFAULT_RATE_LIMIT_POLICY;
    const stateHandle = createStateStore(options.stateDir === undefined ? {} : { paths: statePathsAt(options.stateDir) });
    const state = stateHandle.store;
     ...
     */
    const onRestrictExposure = (intent) => {
