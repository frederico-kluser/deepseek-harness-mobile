...
import { EVENTO_RELATORIO, } from "./events.js";
/* ========================================================================== */
...
function comMarcador(marcador, corpo) {
    return `${marcador}\n${corpo}`;
}
/* ========================================================================== */
 ...
 */
export function comporTextoSessaoNova(evento, agoraMs) {
    const curto = evento.sessao_id_hash.slice(0, HASH_CURTO_LEN);
    return comMarcador(ALERTA_SESSAO_NOVA, `Sessao nova autenticada as ${horario(agoraMs)} — agente ${curto}`);
}
/**
 ...
 */
export function comporTextoAuthFalha(evento, agoraMs) {
    const origem = evento.ip_normalizado === undefined ? '' : ` — de ${evento.ip_normalizado}`;
    return comMarcador(ALERTA_AUTH_FALHA, `Primeira falha de autenticacao na janela de 10 min as ${horario(agoraMs)}${origem}`);
}
/**
 ...
 */
export function comporTextoTunelToggle(evento, agoraMs, url) {
    ...
    if (origem.length === 0) {
        ...
        throw new Error('[guarded-bot] EVENTO_TOGGLE_SEM_ORIGEM: o evento de toggle chegou sem origem ' +
    ...
    return comMarcador(marcador, `Tunel ${verbo} as ${horario(agoraMs)} (origem: ${origem})${linhaUrl}`);
}
/** O TTL expirou: o tunel foi derrubado e as sessoes invalidadas. */
export function comporTextoTTLExpirado(_evento, agoraMs) {
    return comMarcador(ALERTA_TTL_EXPIRADO, `TTL expirado as ${horario(agoraMs)} — o tunel foi derrubado e as sessoes invalidadas.`);
}
/** Entrada em modo restrito: o teto de falhas foi alcancado e o tunel caiu. */
export function comporTextoModoRestrito(_evento, agoraMs) {
    return comMarcador(ALERTA_MODO_RESTRITO, `Modo restrito ativado as ${horario(agoraMs)} — o tunel foi derrubado. A recuperacao e na maquina.`);
}
/**
 ...
 */
export function comporTextoMagicSuspeito(_evento, agoraMs) {
    return comMarcador(ALERTA_MAGIC_SUSPEITO, `Consumo do link magico sem clique detectavel as ${horario(agoraMs)} — o token nao foi usado.`);
}
/**
 ...
 * tunel aberto.
 */
export function comporTextoRelatorio(agoraMs, expiraEm) {
    ...
    return comMarcador(ALERTA_RELATORIO, `O tunel continua aberto — expira as ${horario(expiraEm)} (tempo restante: ${restante}). ` +
 ...
 */
export function comporTextoLinkMagico(agoraMs, urlDoTunel, chave, expiraEm) {
    const link = `${urlDoTunel}?key=${chave}`;
    const validade = expiraEm === undefined
        ? 'válido até você rotacionar'
        : `válido até ${formatarTempoRestante(expiraEm - agoraMs)}`;
    return comMarcador(ALERTA_LINK_MAGICO, `Seu link com a chave de acesso (abre e entra, sem senha):\n${link}\n` +
 ...
 */
export function enviarNotificacao(canal, log, texto) {
    let entregue = false;
    try {
        entregue = canal.send({ v: IPC_PROTOCOL_VERSION, type: 'notify', texto });
    }
    catch (error) {
        log.warn('notificacao proativa nao entregue (o canal lancou; o audit ja foi escrito): ' +
...
export function criarCoalescedor(now, janelaMs = COALESCENCIA_MS) {
 ...
 * mudar nada aqui.
 */
export function criarObservadorSessaoNova(canal) {
    return (evento) => {
 ...
 * deixar o ciclo vivo.
 */
export function criarRelatorioPeriodico(deps) {
    ...
    const bater = () => {
        ...
        try {
            ...
            deps.log.warn('relatorio periodico NAO emitido neste ciclo (projecao do tunel, relogio ou ' +
    ...
    return {
        iniciar() {
            if (disposed || agendado !== undefined)
                return;
            reagendar();
        },
        disposer() {
            ...
            if (agendado !== undefined)
                deps.scheduler.clearTimeout(agendado);
            agendado = undefined;
        },
    };
}
