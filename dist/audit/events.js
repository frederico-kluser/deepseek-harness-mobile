 ...
 */
export function registerSessaoNovaObserver(observador) {
 ...
 */
export function emitSessaoNova(evento, log) {
    ...
    for (const observador of observadores.slice()) {
        try {
            ...
            log.warn('notificacao de sessao nova falhou (best-effort; o audit ja foi escrito): ' +
 ...
 */
export function comporEventoToggle(acao, origem) {
    if (origem.length === 0) {
        throw new Error(`[${PLUGIN_NAME}] EVENTO_TOGGLE_SEM_ORIGEM: a origem do toggle nao pode ser vazia ` +
 ...
 * ninguem.
 */
export function comporEventoReset(origem) {
    if (origem.length === 0) {
        throw new Error(`[${PLUGIN_NAME}] EVENTO_TOGGLE_SEM_ORIGEM: a origem do reset nao pode ser vazia ` +
            '(esperado `telegram:<id>` ou `panel:<idHash>`).');
    }
 ...
 */
export function comporEventoAgenteDespacho(origem, skill) {
    if (origem.length === 0 || skill.length === 0) {
        throw new Error(`[${PLUGIN_NAME}] EVENTO_AGENTE_SEM_ORIGEM: origem e skill do despacho nao podem ser vazias ` +
 ...
 */
export function comporEventoAgenteCancelar(origem, agentId) {
    if (origem.length === 0 || agentId.length === 0) {
        throw new Error(`[${PLUGIN_NAME}] EVENTO_AGENTE_SEM_ORIGEM: origem e agentId do cancelamento nao podem ser vazios.`);
    }
 ...
 */
export function comporEventoAgenteFim(origem, skill, status) {
    if (origem.length === 0 || skill.length === 0) {
        throw new Error(`[${PLUGIN_NAME}] EVENTO_AGENTE_SEM_ORIGEM: origem e skill do fim de run nao podem ser vazias.`);
    }
 ...
 */
export function eventoDoVocabulario(nome) {
    if (NOMES_BASE.has(nome))
        return true;
    const sufixoTtl = sufixoDe(nome, EVENTO_TTL_EXPIRADO);
    if (sufixoTtl !== undefined)
        return SUFIXO_TTL.test(sufixoTtl);
    const sufixoExposicao = sufixoDe(nome, EVENTO_EXPOSICAO_RESTRITA);
    if (sufixoExposicao !== undefined)
        return SUFIXO_NUMERO.test(sufixoExposicao);
    const sufixoLacuna = sufixoDe(nome, EVENTO_LACUNA);
    if (sufixoLacuna !== undefined)
        return SUFIXO_NUMERO.test(sufixoLacuna);
    const sufixoProbe = sufixoDe(nome, EVENTO_PROBE);
    if (sufixoProbe !== undefined)
        return SUFIXO_PROBE.test(sufixoProbe);
    ...
    if (sufixoLigar !== undefined)
        return nome.length > EVENTO_TUNEL_LIGAR.length + 1;
    const sufixoDesligar = sufixoDe(nome, EVENTO_TUNEL_DESLIGAR);
    if (sufixoDesligar !== undefined)
        return nome.length > EVENTO_TUNEL_DESLIGAR.length + 1;
    const sufixoReset = sufixoDe(nome, EVENTO_TUNEL_RESET);
    // A mesma regra dos toggles: `tunel_reset:` (origem vazia) nao designa ninguem.
    if (sufixoReset !== undefined)
        return nome.length > EVENTO_TUNEL_RESET.length + 1;
    ...
    if (sufixoEmergencia !== undefined)
        return nome.length > EVENTO_TUNEL_EMERGENCIA.length + 1;
    const sufixoNaoPareado = sufixoDe(nome, EVENTO_INTENT_NAO_PAREADO);
    if (sufixoNaoPareado !== undefined)
        return nome.length > EVENTO_INTENT_NAO_PAREADO.length + 1;
    ...
    for (const prefixo of [EVENTO_AGENTE_DESPACHO, EVENTO_AGENTE_CANCELAR, EVENTO_AGENTE_FIM]) {
        ...
        if (sufixoAgente !== undefined)
            return nome.length > prefixo.length + 1;
    }
    for (const base of [EVENTO_MAGIC_SUSPEITO, EVENTO_PAINEL_SEGREDO_RECUSA_ANONIMA, EVENTO_PAINEL_CSRF_RECUSADO]) {
        ...
        if (sufixoRajada !== undefined)
            return SUFIXO_RAJADA.test(sufixoRajada);
    }
    return false;
}
/** O que sobra de `nome` depois do prefixo, ou `undefined` se nao comeca por ele. */
function sufixoDe(nome, prefixo) {
