 ...
 */
export type IpcErrorCode = 'SHUTDOWN_IN_PROGRESS' | 'EXPOSURE_DISABLED' | 'RESTRICTED_MODE' | 'PROBE_FAILED' | 'TUNNEL_FAILED' | 'NOT_PAIRED' | 'NONCE_INVALID' | 'RATE_LIMITED' | 'INTERNAL';
/**
 ...
 */
export type IpcIntentName = 'tunnel.up' | 'tunnel.down' | 'tunnel.status' | 'session.issue' | 'secret.rotate' | 'emergency' | 'agent.dispatch' | 'agent.status' | 'agent.cancel' | 'chat.new' | 'worktree.create';
/**
 ...
 */
export type IpcMessageToWorker = IpcStateMessage | IpcAckMessage | IpcErrorMessage | IpcNotifyMessage | IpcPairingChallengeMessage | IpcNonceIssuedMessage | IpcPairingOwnerMessage | IpcAgentReportMessage;
/** worker -> host. A EMENDA-COSTURA-5 acrescentou `nonce.request`. */
export type IpcMessageFromWorker = IpcIntentMessage | IpcNonceRequestMessage | IpcPairingSuccessMessage;
...
export type IpcParseResult = {
    ...
    readonly ok: false;
    readonly reason: 'json-invalido' | 'versao-desconhecida' | 'tipo-desconhecido' | 'forma-invalida';
};
