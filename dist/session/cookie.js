 ...
 */
import { SESSION_ABSOLUTE_TIMEOUT_MS } from "./store.js";
 ...
 */
function ehIpv4Loopback(hostname) {
 ...
 */
export function assertTrustworthyOrigin(origin) {
    if (isTrustworthyOrigin(origin))
        return;
    throw new Error(`[${PLUGIN_NAME}] recusa emitir sessao sobre ${origin.scheme}://${origin.host}: ` +
 ...
 */
export function serializeSessionCookie(id, origin) {
    assertTrustworthyOrigin(origin);
    if (typeof id !== 'string' || !COOKIE_VALUE.test(id)) {
        throw new Error(`[${PLUGIN_NAME}] valor de sessao invalido para cookie: esperado id opaco em base64url`);
    }
...
export function readSessionCookie(cookieHeader) {
    ...
    for (const parte of cookieHeader.split(';')) {
        ...
        if (igual === -1)
            continue;
        if (parte.slice(0, igual).trim() !== SESSION_COOKIE_NAME)
            continue;
        if (achado !== null)
            return null;
        achado = parte.slice(igual + 1).trim();
    }
    if (achado === null || !COOKIE_VALUE.test(achado))
        return null;
    return achado;
}
