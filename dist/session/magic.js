...
function digestOf(mk) {
    return createHash('sha256').update(mk, 'utf8').digest('hex');
}
export function createMagicStore(deps) {
    ...
    return {
        issue() {
            if (disposto) {
                throw new Error(`[${PLUGIN_NAME}] MagicStore ja foi disposto: o link emitido agora nunca seria consumivel`);
            }
            ...
            const redigido = () => ({ mk: '[REDACTED]', expiraEm });
            return {
                mk,
                expiraEm,
                toJSON: redigido,
        ...
        consume(mk) {
            ...
            if (disposto)
                return false;
            if (typeof mk !== 'string' || !PRESENTED_MK.test(mk))
                return false;
            const agora = clock.now();
