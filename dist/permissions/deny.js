 ...
 */
import { safeDecodeURIComponent } from "../http/path.js";
/**
 ...
 */
export function canonicalizePermissionToken(rawToken) {
    let value = rawToken;
 ...
 * `danger-full-access`.
 */
export function requestsDeniedPermission(command, deniedPermissions) {
    const tokens = new Set(command
        .split(/[^A-Za-z0-9._%+-]+/u)
        .map((token) => canonicalizePermissionToken(token))
        .filter((token) => token.length > 0));
    for (const permission of deniedPermissions) {
        const needle = canonicalizePermissionToken(permission);
        if (needle.length > 0 && tokens.has(needle))
            return permission;
    }
    return undefined;
}
