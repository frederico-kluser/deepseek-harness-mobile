...
import { probeHttp, TunnelError } from "./discover.js";
/* ========================================================================== */
 ...
 */
export function isApplicationResponse(status) {
...
export const defaultReadinessDeps = {
    now: Date.now,
    sleep: async (ms, signal) => {
        await timersSleep(ms, undefined, { signal });
    },
    probeUrl: (target, signal, timeoutMs) => probeHttp({ target, signal, timeoutMs, maxBodyBytes: DISCARD_BODY }),
    pollIntervalMs: 500,
 ...
 */
function parseUsableUrl(url) {
    let parsed;
    try {
        parsed = new URL(url);
    }
    catch (cause) {
        throw new TunnelError('INVALID_CONFIG', 'O endereco entregue a verificacao de disponibilidade nao e um endereco ' +
            'absoluto valido. Quem chama tem de passar o resultado de `discover()`, ' +
            'que ja vem normalizado com esquema.', false, { cause });
    }
    if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') {
        throw new TunnelError('INVALID_CONFIG', 'O endereco entregue a verificacao de disponibilidade nao usa HTTP nem ' +
            'HTTPS. Quem chama tem de passar o resultado de `discover()`, que ja vem ' +
            'normalizado com esquema.', false);
    }
    ...
    if (parsed.username !== '' || parsed.password !== '') {
        throw new TunnelError('INVALID_CONFIG', 'O endereco do tunel traz credencial embutida (a forma ' +
            ...
            'do plugin, nao o endereco.', false);
    }
    return parsed;
}
function assertUsableTimeout(timeoutMs) {
    if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > MAX_READINESS_TIMEOUT_MS) {
        throw new TunnelError('INVALID_CONFIG', 'O prazo da verificacao de disponibilidade tem de ser um numero inteiro de ' +
            ...
            'observado. Corrija o valor em vez de esperar que ele seja corrigido por si.', false);
    }
}
/* ========================================================================== */
/* Espera                                                                     */
 ...
 */
export function createTunnelReadiness(deps = defaultReadinessDeps) {
    async function waitUntilUsable(input) {
        ...
        for (;;) {
            ...
            if (aborted())
                return { usable: false, status: lastStatus };
            const probe = await deps.probeUrl(target, signal, deps.attemptTimeoutMs);
            if (aborted())
                return { usable: false, status: lastStatus };
            if (probe.kind === 'response') {
                lastStatus = probe.status;
                if (isApplicationResponse(probe.status))
                    return { usable: true, status: probe.status };
            }
