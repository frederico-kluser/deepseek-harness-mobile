...
import { isProcessAlive, programOf, readProcessCmdline, readProcessStartMs, START_TIME_TOLERANCE_MS, } from "../proc/introspect.js";
import { treeKill } from "../proc/tree-kill.js";
import { applyTtlExpiry, decideOnResume, ttlDeadline, } from "./ttl.js";
/**
 ...
 */
export { readProcessCmdline, readProcessStartMs, START_TIME_TOLERANCE_MS } from "../proc/introspect.js";
/** Escreve o registo. Chamado logo apos o `spawn`, no gancho `onSpawned`. */
export function recordTunnelProcess(store, record) {
    store.update((state) => ({
        ...state,
 ...
 */
export function clearTunnelProcess(store) {
    store.update((state) => ({ ...state, tunnel: undefined }));
}
export function readTunnelProcess(store) {
 ...
 */
export function looksLikeCloudflared(cmdline, expectedCommand) {
    ...
    if (expected !== undefined && expected.length > 0) {
        ...
        if (program === expected || basename(program) === basename(expected))
            return true;
        /**
        ...
        if (second !== undefined && second === expected)
            return true;
    }
    return /(^|[\s/\\])cloudflared(\s|$|\.exe)/iu.test(program);
}
/** Dependencias reais da varredura, no processo corrente. */
export function defaultOrphanSweepDeps(store, log, expectedCommand) {
    return {
        store,
        log,
        expectedCommand,
 ...
 */
export function sweepOrphanTunnel(deps) {
    ...
    if (!Number.isInteger(record.pid) || record.pid <= 1 || record.pid === process.pid) {
        deps.log.warn(`registo de tunel com pid improvavel (${String(record.pid)}); apagado sem derrubar nada.`);
    ...
    if (cmdline !== null && !looksLikeCloudflared(cmdline, deps.expectedCommand)) {
        deps.log.warn('o pid registado como tunel pertence agora a OUTRO programa (pid reutilizado); ' +
    ...
    if (startedAt !== null && startedAt > record.startedAt + START_TIME_TOLERANCE_MS) {
        deps.log.warn('o pid registado esta ocupado por um processo que arrancou DEPOIS do registo: ' +
            'e outra instancia, nao a nossa. Nada foi derrubado e o registo foi limpo.');
        clearTunnelProcess(deps.store);
        return { outcome: 'foreign', record };
    }
    deps.log.warn('tunel orfao de uma execucao anterior encontrado VIVO no arranque; a derrubar ' +
...
function signalGroup(deps, pid, signal) {
...
export function ownerOrphanMessage() {
    return ('No arranque foi encontrado um tunel de uma execucao anterior ainda vivo, sem o portao ' +
 ...
 */
export function recoverTunnelAtBoot(deps) {
    const sweep = sweepOrphanTunnel(deps.sweep);
    if (sweep.record === undefined)
        return { sweep, ttl: 'sem-registo' };
    ...
    try {
        ...
        deps.log.error(`orfao derrubado e sessoes invalidadas, mas o registo de auditoria FALHOU: ${error instanceof Error ? error.message : String(error)}`);
    }
    try {
        deps.effects.notifyOwner(ownerOrphanMessage(), facts);
    }
    catch (error) {
        deps.log.error(`orfao derrubado, mas o aviso ao dono FALHOU: ${error instanceof Error ? error.message : String(error)}`);
    }
    return { sweep, ttl: 'dentro-do-prazo' };
}
