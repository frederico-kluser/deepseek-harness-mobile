...
import { SpawnSpecError } from "../proc/failure.js";
 ...
 */
const LOGLEVEL_FLAG_NAMES = new Set(['loglevel', 'log-level']);
/** Niveis que registam URLs, metodos e TODOS os cabecalhos. */
const FORBIDDEN_LOGLEVELS = new Set(['debug', 'trace']);
/** `--token` entrega o segredo em `argv`. `--token-file` entrega um CAMINHO. */
const FORBIDDEN_FLAG_NAMES = new Set(['token']);
/**
...
export function resolveCloudflaredCommand(binaryPath) {
 ...
 * deixa de ser prova.
 */
export function originPortOfOwnServer(server) {
    if (server === undefined || server === null) {
        throw new SpawnSpecError('o alvo do tunel precisa de ser um servidor HTTP deste processo; nao foi fornecido nenhum.');
    }
    if (!server.listening) {
        throw new SpawnSpecError('o servidor alvo do tunel nao esta em escuta; sem `listen()` bem sucedido nao ha prova de posse da porta.');
    }
    ...
    if (address === null || typeof address === 'string') {
        ...
        throw new SpawnSpecError('o servidor alvo do tunel nao esta ligado a uma porta TCP; nao ha origem que se possa publicar.');
    }
 ...
 */
export function buildCloudflaredArgv(input) {
    ...
    if (input.mode === 'named') {
        ...
        if (tokenFile === undefined || tokenFile.length === 0) {
            throw new SpawnSpecError('tunnel.mode e `named` mas nao ha `tunnel.tokenFile`. O token de um named tunnel ' +
...
function tokenizeArgv(argv) {
    ...
    for (const argument of argv) {
        ...
        if (separator !== -1)
            tokens.push({ flag: null, value: argument.slice(separator + 1) });
    }
    return tokens;
}
/**
...
export function assertNoForbiddenArgv(argv) {
    ...
    for (const [index, token] of tokens.entries()) {
        ...
        if (FORBIDDEN_FLAG_NAMES.has(token.flag)) {
            throw new SpawnSpecError('o argv do cloudflared nao pode conter `--token`: o `argv` e legivel por ' +
                'qualquer processo do mesmo utilizador em `/proc/<pid>/cmdline` e no `ps`. ' +
                'O token de um named tunnel entra por `--token-file`.');
        }
        if (!LOGLEVEL_FLAG_NAMES.has(token.flag))
            continue;
        const level = (tokens[index + 1]?.value ?? '').toLowerCase();
        if (!FORBIDDEN_LOGLEVELS.has(level))
            continue;
        throw new SpawnSpecError(`o argv do cloudflared nao pode pedir nivel de log \`${level}\`: ` +
...
function assertUsablePort(port, field) {
 ...
 */
export function toTunnelFailure(processFailure) {
    switch (processFailure.kind) {
        case 'BINARY_NOT_FOUND':
            return {
                code: 'BINARY_NOT_FOUND',
                message: 'cloudflared nao encontrado. Instale-o pelo repositorio oficial da Cloudflare ' +
        ...
        case 'BINARY_NOT_EXECUTABLE':
            return {
                ...
                message: 'o cloudflared existe mas esta conta nao o pode executar. ' +
