 ...
 */
export function probeHttp(options) {
...
export function quickTunnelUrl(metricsPort) {
 ...
 */
export function createStderrScanner(stream) {
    let buffer = '';
    let found = null;
    let failed = null;
    let disposed = false;
...
export const defaultDiscoveryDeps = {
    ...
    probeQuickTunnel: (metricsPort, signal, timeoutMs) => probeHttp({
 ...
 */
export function readQuickTunnelBody(body) {
    let parsed;
    try {
...
/* Descoberta                                                                 */
/* ========================================================================== */
function assertUsableInput(input) {
    const { metricsPort, timeoutMs } = input;
    if (!Number.isInteger(metricsPort) || metricsPort < 1 || metricsPort > 65_535) {
        throw invalidInput('A porta do servidor de metricas do cloudflared nao foi fixada. ' +
    ...
    if (!Number.isFinite(timeoutMs) || timeoutMs < MIN_DISCOVERY_TIMEOUT_MS) {
        throw invalidInput(`O prazo de descoberta tem de ser um numero finito de pelo menos ${String(MIN_DISCOVERY_TIMEOUT_MS)} ms. ` +
...
function timeoutFailure(timeoutMs, attempts, lastMetricsReason, stderrFailure) {
    const stderrNote = stderrFailure === null
        ? ''
        : ` O fluxo de saida do processo falhou (${stderrFailure}), pelo que a leitura por log tambem nao pode acontecer.`;
    return new TunnelError('READINESS_TIMEOUT', `O tunel arrancou mas nao publicou nenhuma URL em ${String(Math.round(timeoutMs / 1000))} s. ` +
        ...
        'a interface local enquanto investiga.', true);
}
function processExitedFailure(cause) {
    return new TunnelError('PROCESS_EXITED', 'O processo do tunel terminou durante o arranque, antes de publicar qualquer ' +
        ...
        'bloqueada. O acesso remoto vai ser tentado de novo.', true, cause === undefined ? undefined : { cause });
}
/**
 ...
 */
export function createTunnelDiscovery(deps = defaultDiscoveryDeps) {
            ...
            for (;;) {
                ...
                if (probe.kind === 'response') {
                    if (probe.status === 200) {
                        ...
                        if (reading.kind === 'url')
                            return { url: reading.url, via: 'metrics' };
                ...
                if (fromStderr !== null)
                    return { url: fromStderr, via: 'stderr' };
                ...
                try {
                    ...
                    if (aborted())
                        throw processExitedFailure(error);
                    throw error;
                }
            }
        }
