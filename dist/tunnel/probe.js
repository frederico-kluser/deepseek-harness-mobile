 ...
 */
export function buildProbePlan(canaryToken, apiReadPath) {
    return [
 ...
 */
export async function runGateProbe(input) {
...
export function describeFailure(outcome) {
    return {
        code: 'PROBE_FAILED',
        probe: outcome.probe,
        message: `o tunel NAO subiu: a sonda \`${outcome.probe}\` ${describeObservation(outcome)}, ` +
...
function describeObservation(outcome) {
    if (outcome.status === null)
        return 'nao pode ser medida (socket fechado, timeout ou erro de rede)';
 ...
 */
export function auditProbeDecision(audit, result) {
 ...
 */
export function createHttpProbeTransport(options) {
    return {
        send(probeRequest, signal) {
