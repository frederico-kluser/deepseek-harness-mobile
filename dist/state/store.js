...
import { closeSync, constants, fchmodSync, fstatSync, fsyncSync, openSync, readFileSync, renameSync, unlinkSync, writeSync, } from 'node:fs';
import { join } from 'node:path';
import { EXPOSURE_MASK, STATE_FILE_MODE, assertOwnedByUs, ensureStateDir, resolveStatePaths, } from "./paths.js";
import { StateError, corruptStateError, emptyState, parsePersistedState, parseStateDocument, serializeStateDocument, } from "./schema.js";
/**
 ...
 */
export function assertNotExposed(mode, file) {
    if ((mode & EXPOSURE_MASK) === 0)
        return;
    throw new StateError('STATE_MODE_TOO_OPEN', `${file} tem modo ${octal(mode)} e o exigido e ${octal(STATE_FILE_MODE)}: ` +
 ...
 */
export function readStateFrom(paths) {
    let fd;
    let text;
    try {
        ...
        if (code === 'ENOENT' || code === 'ENOTDIR')
            return emptyState();
        if (code === 'ELOOP') {
            throw new StateError('STATE_DIR_UNSAFE', `${paths.file} e um link simbolico. Um estado por link deixa quem ` +
...
export function createStateStore(options = {}) {
    ...
    function assertLive() {
        if (disposed) {
            throw new StateError('STATE_STORE_DISPOSED', 'este StateStore ja foi disposto. Depois do disposer, ler ou escrever ' +
    ...
    function writeAtomic(payload) {
        ...
        pending.add(tmp);
        try {
            const fd = openSync(tmp, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, STATE_FILE_MODE);
            try {
            ...
            if (error instanceof StateError)
                throw error;
            throw new StateError('STATE_WRITE_FAILED', `falhou a escrita atomica de ${paths.file}: ${messageOf(error)}. ` +
    ...
    const store = {
        ...
        update(fn) {
            assertLive();
            if (updating) {
                throw new StateError('STATE_REENTRANT_UPDATE', 'update() foi chamado de dentro do proprio callback de update(). A ' +
            ...
            try {
                ...
                if (next === undefined || next === null) {
                    throw corruptStateError(paths.file, 'o callback de update() nao devolveu estado nenhum (esqueceu-se do `return`?)');
                }
