 ...
 */
import { comporEventoReset, comporEventoToggle } from "../audit/events.js";
import { comporTextoTunelToggle, enviarNotificacao } from "../audit/notify.js";
...
export function createTunnelController(deps) {
    ...
    let observado = { state: 'STOPPED', attempts: 0 };
    ...
    /* ------------------------------------------------------------------ */
    const auditar = (action, requestedBy, resultado) => {
        ...
        try {
            ...
            log.error(`auditoria de ${eventoDe(action)} RECUSADA (origem vazia): ` +
        ...
        try {
            ...
            log.error(`falha ao registar auditoria de ${eventoDe(action)} (${requestedBy}): ` +
                `${error instanceof Error ? error.message : String(error)}`);
     ...
     */
    const notificarToggle = (action, nome, resultado) => {
        if (action === 'reset' || resultado !== 'permitido')
            return;
        if (deps.canalNotificacao === undefined)
            return;
        try {
            ...
            const evento = { evento: nome, resultado: 'permitido' };
            const texto = comporTextoTunelToggle(evento, deps.agora(), observado.info?.url);
            enviarNotificacao(deps.canalNotificacao, log, texto);
        }
        catch (error) {
            log.error(`notificacao de ${eventoDe(action)} falhou (best-effort; o audit ja foi escrito): ` +
                `${error instanceof Error ? error.message : String(error)}`);
        }
    };
    const difundir = () => {
    ...
    const reconciliar = () => {
        ...
        switch (snap.state) {
            ...
            case 'DEGRADED':
                ...
                if (estado === 'STARTING' || estado === 'READY')
                    transicionar('DEGRADED');
                break;
            case 'FAILED':
    ...
    /* ------------------------------------------------------------------ */
    const registrarJanela = (requestId, resultado) => {
        ...
        if (janela.size > MAX_JANELA_IDEMPOTENCIA) {
            ...
            if (maisAntigo !== undefined)
                janela.delete(maisAntigo);
        }
    };
    /* ------------------------------------------------------------------ */
    /* A avaliacao das intents                                             */
    /* ------------------------------------------------------------------ */
    const recusar = (recusa, intent) => {
     ...
     */
    const confirmacaoExigida = (intent) => deps.requerConfirmacao && intent.requestedBy !== ORIGEM_BOOT;
    const verificarNonce = (intent, action) => {
        if (!confirmacaoExigida(intent))
            return null;
        if (intent.nonce === undefined || intent.nonce.length === 0)
    ...
    const avaliarStart = (intent) => {
        // Pre-condicoes da tabela (a mais fechada primeiro).
        if (deps.restritoAtivo())
            return recusar('MODO_RESTRITO', intent); // CTL-015
        if (!deps.segredoForte())
            return recusar('SEM_SEGREDO_FORTE', intent); // CTL-009
        switch (estado) {
            case 'STOPPED': {
    ...
    const avaliarStop = (intent, aguardarStart) => {
        ...
        if (aguardarStart && startsPendentes > 0)
            return null;
        switch (estado) {
            case 'STOPPED':
    ...
    const decidir = (intent, aguardarStart) => {
     ...
     */
    const concluirStart = (intent, snap) => {
        observado = snap;
        if (disposed)
            return { estado: 'STOPPED', idempotente: false };
    ...
    const executarStart = async (intent) => {
        try {
    ...
    const processarFila = async (intent) => {
        try {
            ...
            if (anterior !== undefined)
                return { ...anterior, idempotente: true };
    ...
    const despachar = (intent) => {
        ...
        const anterior = janela.get(intent.requestId);
        if (anterior !== undefined)
            return Promise.resolve({ ...anterior, idempotente: true });
        if (intent.action === 'start' && estado === 'STOPPING') {
            const resultado = recusar('SHUTDOWN_IN_PROGRESS', intent);
            registrarJanela(intent.requestId, resultado);
        ...
        cadeia = promessa.then(() => undefined, () => undefined);
        return promessa;
    };
    return {
        decidirSincrono,
        despachar,
        emitirNonce: (action) => deps.confirm.issue(action),
        snapshot: () => ({
            state: estado,
