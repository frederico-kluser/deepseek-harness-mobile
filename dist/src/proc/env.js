 ...
 */
export function resolverProvedorDoAmbiente(ambiente = process.env) {
    const bruto = ambiente[WORKER_PROVIDER_ENV_VAR]?.trim();
    if (bruto === undefined || bruto === '')
        return DEFAULT_PROVIDER;
    if (!(bruto in PROVIDER_ENV)) {
        throw new Error(`${WORKER_PROVIDER_ENV_VAR}='${bruto}' nao e um provedor conhecido ` +
            `(${Object.keys(PROVIDER_ENV).join(' | ')}). Corrija a variavel e repita.`);
    }
    return bruto;
}
export function buildWorkerEnv(source, token, provider = DEFAULT_PROVIDER) {
    const env = {};
    for (const [key, value] of Object.entries(source)) {
        if (value === undefined)
            continue;
        const upper = key.toUpperCase();
        const allowed = WORKER_ENV_ALLOWLIST.includes(upper) ||
