...
import { formatAuditLine } from "./format.js";
...
const OPEN_FLAGS = constants.O_WRONLY | constants.O_CREAT | constants.O_APPEND | constants.O_NOFOLLOW;
/**
...
export class AuditWriteError extends Error {
    ...
    constructor(message, perdidos, path, options) {
...
function requireAbsolute(value, origem) {
    if (!isAbsolute(value)) {
...
export function resolveAuditLogPath(env = process.env, home = homedir(), configuredHome) {
...
export function openAuditLog(options = {}) {
    ...
    const emitir = (line) => {
        ...
        try {
            ...
            while (written < buffer.length) {
                ...
                if (n <= 0)
                    throw new Error(`escrita curta sem progresso (${String(n)} bytes)`);
                written += n;
            }
        }
        catch (cause) {
            ...
            if (written > 0)
                repairTornLine(fd, write);
            throw cause;
        }
        if (options.fsyncEachWrite === true)
            fsyncSync(fd);
    };
    return {
        path,
        append(event) {
            if (closed)
                throw new AuditWriteError('log de auditoria ja fechado', perdidos, path);
            const ts = now();
            try {
                if (perdidos > 0) {
                ...
                perdidos += 1;
                throw new AuditWriteError(`nao foi possivel registar a auditoria — o gate TEM de negar (fail-closed). ` +
                    `Registos perdidos nesta janela: ${String(perdidos)}.`, perdidos, path, { cause });
            }
        },
        perdidos: () => perdidos,
        dispose() {
...
function assertOutsideWorkspace(path, workspaceRoot) {
    if (path === workspaceRoot || path.startsWith(workspaceRoot + sep)) {
        throw new AuditOpenError(`o log de auditoria nao pode ficar dentro do workspace (${workspaceRoot}): ${path}. ` +
...
function assertDirIsOurs(dir) {
    const st = lstatSync(dir);
    if (st.isSymbolicLink()) {
        throw new AuditOpenError(`${dir} e um symlink — recuso guardar o log atras de um redirecionamento`);
    }
    if ((st.mode & 0o077) !== 0) {
        throw new AuditOpenError(`o diretorio ${dir} esta aberto a grupo/outros (${(st.mode & 0o777).toString(8)}). ` +
...
function assertOwnerOnly(mode, path) {
    ...
    if ((bits & 0o177) !== 0) {
        throw new AuditOpenError(`modo ${bits.toString(8).padStart(4, '0')} e mais frouxo que 0600 em ${path}. ` +
