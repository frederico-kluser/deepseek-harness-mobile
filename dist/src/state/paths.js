...
import { StateError } from "./schema.js";
/* ========================================================================== */
...
export function resolveDshHome(options = {}) {
    ...
    if (configured !== undefined)
        return assertAbsolute(expandHomePath(configured, home), 'config.stateHome');
    const fromEnv = present(env[DSH_HOME_ENV]);
    if (fromEnv !== undefined)
        return assertAbsolute(expandHomePath(fromEnv, home), `$${DSH_HOME_ENV}`);
    return assertAbsolute(join(home, DSH_HOME_DIR_NAME), '$HOME');
}
function assertAbsolute(path, origin) {
    if (!isAbsolute(path)) {
        throw new StateError('STATE_PATH_INVALID', `${origin} = ${JSON.stringify(path)} nao e um caminho absoluto. ` +
...
export function statePathsAt(dir) {
...
export function ensureStateDir(paths) {
    ...
    if (stat.isSymbolicLink()) {
        throw new StateError('STATE_DIR_UNSAFE', `${paths.dir} e um link simbolico. O diretorio de estado tem de ser um ` +
 ...
 */
export function assertOwnedByUs(uid, path) {
    const ours = process.getuid?.();
    if (ours !== undefined && uid !== ours) {
        throw new StateError('STATE_NOT_OWNED', `${path} pertence ao uid ${uid}, e este processo corre como uid ${ours}. ` +
