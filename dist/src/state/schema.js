 ...
 */
import { BrandError, toSecretDigest } from "../brand.js";
 ...
 */
export function corruptStateError(source, reason) {
    return new StateError('STATE_CORRUPT', `${source} existe mas nao e um estado valido: ${reason}. ` +
...
/* -------------------------------------------------------------------------- */
function asRecord(value, source, path) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        throw corruptStateError(source, `${path} tinha de ser um objeto`);
    }
    return value;
}
function asInteger(value, source, path, min) {
    if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < min) {
        throw corruptStateError(source, `${path} tinha de ser um inteiro >= ${min}`);
    }
    return value;
}
function asLiteral(value, allowed, source, path) {
    if (typeof value !== 'string' || !allowed.includes(value)) {
        throw corruptStateError(source, `${path} tinha de ser um de ${allowed.map((v) => `'${v}'`).join(' | ')}`);
    }
    return value;
}
function rejectUnknownKeys(record, known, source, path) {
    for (const key of Object.keys(record)) {
        if (!known.has(key)) {
            throw corruptStateError(source, `${path} tem a chave desconhecida ${JSON.stringify(key)} — a proxima ` +
...
/* Sub-objetos                                                                */
/* -------------------------------------------------------------------------- */
const RESTRICTED_KEYS = new Set(['since', 'reason']);
const TUNNEL_KEYS = new Set(['pid', 'startedAt', 'mode']);
const PAIRING_KEYS = new Set(['ownerUserId', 'ownerChatId', 'pairedAt']);
function parseRestricted(value, source) {
...
function parseTunnel(value, source) {
 ...
 */
function normalizarEixoDoDono(value, source, path, legado) {
    if (typeof value === 'number')
        return String(legado(value));
    ...
    throw corruptStateError(source, `${path} tinha de ser um inteiro (formato legado v1) ou uma string nao vazia (formato v2)`);
}
function parsePairing(value, source) {
    const record = asRecord(value, source, 'pairing');
    rejectUnknownKeys(record, PAIRING_KEYS, source, 'pairing');
    return {
        ownerUserId: normalizarEixoDoDono(record['ownerUserId'], source, 'pairing.ownerUserId', (v) => asInteger(v, source, 'pairing.ownerUserId', 1)),
        ...
        ownerChatId: normalizarEixoDoDono(record['ownerChatId'], source, 'pairing.ownerChatId', (v) => asInteger(v, source, 'pairing.ownerChatId', Number.MIN_SAFE_INTEGER)),
 ...
 */
export const PROVIDER_LITERALS = ['telegram'];
function parseProvider(value, source) {
    if (value === undefined || value === null)
        return undefined;
    return asLiteral(value, PROVIDER_LITERALS, source, 'provider');
}
function parseSecretDigest(value, source) {
    if (typeof value !== 'string') {
    ...
    try {
        ...
        if (error instanceof BrandError)
            throw corruptStateError(source, `secretDigest invalido (${error.message})`);
        throw error;
    }
}
/* ========================================================================== */
 ...
 */
export function migrateState(raw, source) {
    const record = asRecord(raw, source, 'o documento');
    rejectUnknownKeys(record, KNOWN_KEYS, source, 'o documento');
    const version = record['version'];
    if (typeof version !== 'number' || !Number.isSafeInteger(version) || version < 1) {
        throw corruptStateError(source, 'falta o campo `version` (inteiro >= 1) — sem ele nao se sabe que forma o ficheiro tem');
    }
    if (version > CURRENT_STATE_VERSION) {
        throw new StateError('STATE_VERSION_UNSUPPORTED', `${source} declara \`version: ${version}\` e este plugin so sabe ler ate ` +
 ...
 */
export function parsePersistedState(value, source) {
 ...
 */
export function serializeStateDocument(state) {
    const ordered = {
        version: state.version,
        desiredState: state.desiredState,
    };
    if (state.secretDigest !== undefined)
        ordered['secretDigest'] = state.secretDigest;
    ...
    if (state.provider !== undefined)
        ordered['provider'] = state.provider;
 ...
 */
export function parseStateDocument(text, source) {
