...
function payloadBits(text, alnum) {
    if (alnum)
        return 11 * Math.floor(text.length / 2) + 6 * (text.length % 2);
    return 8 * Buffer.byteLength(text, 'utf8');
}
/** Escreve modo + contagem + dados. Contagem: 9 bits (alnum) ou 8 (byte) ate a versao 9. */
function writeSegment(buffer, text, alnum) {
    if (alnum) {
        ...
        if (text.length % 2 === 1)
            buffer.push(ALPHANUMERIC.indexOf(text[text.length - 1]), 6);
        return;
    }
...
function toDataCodewords(buffer, spec) {
...
function reedSolomon(block, ecLen) {
    ...
    for (let i = 0; i < ecLen; i += 1) {
        for (let j = i + 1; j > 0; j -= 1)
            generator[j] = generator[j - 1] ^ gfMul(generator[j], GF_EXP[i]);
    ...
    for (const byte of block) {
        ...
        for (let j = 0; j < ecLen; j += 1)
            remainder[j] = remainder[j] ^ gfMul(generator[ecLen - 1 - j], factor);
    }
    return remainder;
}
...
function place(canvas, y, x, value) {
...
function functionPatterns(spec) {
    const size = spec.version * 4 + 17;
    const canvas = { size, dark: new Uint8Array(size * size), fixed: new Uint8Array(size * size) };
    ...
    for (let i = 0; i <= 8; i += 1) {
        if (canvas.fixed[8 * size + i] === 0)
            place(canvas, 8, i, 0);
        if (canvas.fixed[i * size + 8] === 0)
            place(canvas, i, 8, 0);
    }
    for (let i = 0; i < 8; i += 1) {
        if (canvas.fixed[8 * size + (size - 1 - i)] === 0)
            place(canvas, 8, size - 1 - i, 0);
        if (canvas.fixed[(size - 1 - i) * size + 8] === 0)
            place(canvas, size - 1 - i, 8, 0);
    }
    return canvas;
}
/** Zigue-zague de baixo para cima, duas colunas de cada vez, saltando a coluna 6. */
function placeCodewords(canvas, codewords) {
...
function formatBits(mask) {
    ...
    for (let i = 14; i >= 10; i -= 1)
        if ((rest >>> i) & 1)
            rest ^= 0b101_0011_0111 << (i - 10);
    return ((data << 10) | rest) ^ 0b101_0100_0001_0010;
}
function writeFormat(canvas, mask) {
    const size = canvas.size;
    ...
    for (let i = 0; i < 15; i += 1) {
        ...
        if (i < 8)
            canvas.dark[8 * size + (size - 1 - i)] = bit;
        else
            canvas.dark[(size - 15 + i) * size + 8] = bit;
    }
}
/**
 ...
 */
function runLengths(size, at) {
    const runs = at(0) === 1 ? [-1, 1] : [1];
    let previous = at(0);
 ...
 */
function calcN1N3(runs) {
    let demerit = 0;
    for (let i = 0; i < runs.length; i += 1) {
        const run = runs[i];
        if (run >= 5)
            demerit += 3 + (run - 5);
        if ((i & 1) === 0 || i < 3 || i >= runs.length - 2 || run % 3 !== 0)
            continue;
        const fact = run / 3;
        ...
        if (i === 3 || runs[i - 3] >= 4 * fact)
            demerit += 40;
        else if (i + 4 >= runs.length || runs[i + 3] >= 4 * fact)
            demerit += 40;
    }
    return demerit;
}
/** Demerito total da matriz ja mascarada e com a informacao de formato escrita. */
function penalty(canvas) {
    const size = canvas.size;
    const at = (y, x) => canvas.dark[y * size + x];
    let demerit = 0;
    let dark = 0;
    for (let y = 1; y < size; y += 1) {
        for (let x = 1; x < size; x += 1) {
            ...
            if (v === at(y - 1, x) && v === at(y, x - 1) && v === at(y - 1, x - 1))
                demerit += 3;
        }
    }
...
export function encodeQr(text) {
    ...
    if (spec === undefined) {
        throw new QrEncodeError(`entrada de ${text.length} caracteres nao cabe num QR nivel M versao 6 (${needed} bits > ${SPECS[5].dataCodewords * 8})`);
    }
