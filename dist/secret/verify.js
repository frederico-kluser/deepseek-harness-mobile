...
import { canonicalizeSecret } from "./canonical.js";
 ...
 */
export function verifySecret(candidate, expected) {
    if (expected === undefined)
        return false;
    const stored = Buffer.from(expected, 'hex');
    if (stored.length !== SECRET_DIGEST_BYTES) {
        throw new SecretDigestError(`digest guardado tem ${stored.length} bytes, esperados ${SECRET_DIGEST_BYTES} (sha256 em hex de 64 caracteres)`);
    }
