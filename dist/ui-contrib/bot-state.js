 ...
 */
export function derivarEstadoDoBot(input) {
    if (!input.tokenConfigurado)
        return { online: false, motivo: 'sem-chave' };
    if (input.pairing === undefined)
        return { online: false, motivo: 'sem-pareamento' };
    return { online: true };
}
/**
 ...
 */
export function passosDoBot(estado) {
    if (!estado.online)
        return PASSOS_DO_CONECTOR;
    return PASSOS_DE_USO;
}
const D = (titulo, texto) => ({ titulo, texto });
/** Ligar um bot do Telegram, passo a passo — sem segredo nenhum. */
export const PASSOS_DO_CONECTOR = [
    D('Crie o bot no Telegram', `Abra o Telegram e escreva ao @BotFather (a conta oficial para criar bots). ` +
    ...
    D('Guarde a chave', `No terminal desta maquina, escreva:\n\n    ${COMANDO_CLI} --pedir-token\n\n` +
    ...
    D('Peca o codigo de pareamento', `Ainda no terminal, escreva:\n\n    ${COMANDO_CLI} --parear\n\n` +
        `Isso mostra UM CODIGO de pareamento, apenas desse terminal, valido por ` +
        `alguns minutos. Nao o reencaminhe a ninguem.`),
    D('Envie o codigo no bot', `No Telegram, abra a conversa com o seu bot (se nao existir, toque em ` +
...
export const PASSOS_DE_USO = [
    D('Ligar / desligar', `No bot, envie /ligar (desde esta superficie o tunel liga pelo clique) e ` +
        `/desligar para o derrubar. No Telegram tambem: /ligar e /desligar.`),
    D('Acesso', `Envie /acessar no bot para receber um convite de sessao para esta maquina.`),
];
